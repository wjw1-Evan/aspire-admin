...
**2025-01-XX**: 清理规则文件中的过时代码示例和重复内容。
- ✅ 修复 `business-logic.mdc`、`new-entity-checklist.mdc`、`design-patterns-architecture.mdc`、`performance-optimization.mdc` 中的过时代码示例（移除 `SetCurrentTimestamp()` 和 `SetOperationTracking()` 的使用）
- ✅ 更新所有示例代码，确保符合 `core-backend-standards.mdc` 中的规范（只设置业务字段，审计字段自动填充）
- ✅ 移除过时的操作日志记录代码示例
