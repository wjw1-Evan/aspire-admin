...
在使用 Playwright 进行测试前，需要先启动 Aspire 开发环境：
...
2. **等待服务启动**
   - 等待后端服务完全启动（通常需要 1-2 分钟）
   - 等待前端服务完全启动（通常需要 30 秒 - 1 分钟）
   - 确认所有服务正常运行

3. **验证服务状态**
   - 前端地址：`http://localhost:15001/`
   - 后端 API：`http://localhost:15000/`
   - 确保可以正常访问前端页面

### 9.2 测试账号

系统提供默认测试账号用于 Playwright 测试：
...
test.describe('登录功能测试', () => {
  test('成功登录系统', async ({ page }) => {
    // 访问登录页面
    await page.goto('http://localhost:15001/');
...
```typescript
test('密码本页面加载测试', async ({ page }) => {
  await page.goto('http://localhost:15001/password-book');
  await expect(page.locator('.password-book-list')).toBeVisible();
});
```
...
1. **等待策略**
   - 使用 `await expect().toBeVisible()` 等待元素可见
   - 避免使用固定 `sleep()`，使用 Playwright 的自动等待机制
...
3. **测试隔离**
   - 每个测试用例独立运行，不依赖其他测试
   - 使用 `beforeEach` 和 `afterEach` 进行测试前后的清理
...
4. **浏览器兼容性**
   - 测试多浏览器兼容性（Chrome、Firefox、Safari）
   - 使用 Playwright 的多浏览器支持
