export default {
  'menu.welcome': 'Selamat Datang',
  'menu.more-blocks': 'Blocks Lainnya',
  'menu.home': 'Halaman Awal',
  'menu.admin': 'Admin',
  'menu.admin.sub-page': 'Sub-Halaman',
  'menu.login': 'Masuk',
  'menu.register': 'Pendaftaran',
  'menu.register-result': 'Hasil Pendaftaran',
  'menu.system': 'Manajemen Sistem',
  'menu.system.user-management': 'Manajemen Pengguna',
  'menu.system.role-management': 'Manajemen Peran',
  'menu.system.menu-management': 'Manajemen Menu',
  'menu.system.user-log': 'Log Pengguna',
  'menu.system.company-settings': 'Pengaturan Perusahaan',
  'menu.system.company-management': 'Manajemen Perusahaan',
  'menu.system.my-activity': 'Aktivitas Saya',
  'menu.project-management': 'Manajemen Proyek',
  'menu.project-management.task': 'Manajemen Tugas',
  'menu.project-management.project': 'Daftar Proyek',
  'menu.task-management': 'Manajemen Tugas',
  'menu.iot-platform': 'Platform IoT',
  'menu.iot-platform.gateway': 'Manajemen Gateway',
  'menu.iot-platform.device': 'Manajemen Perangkat',
  'menu.iot-platform.datapoint': 'Manajemen Titik Data',
  'menu.iot-platform.event': 'Acara & Peringatan',
  'menu.xiaoke-management': 'Manajemen Xiaoke',
  'menu.xiaoke-management.config': 'Manajemen Konfigurasi',
  'menu.xiaoke-management.chat-history': 'Manajemen Riwayat Chat',
  'menu.company-search': 'Cari Perusahaan',
  'menu.my-join-requests': 'Permintaan Bergabung Saya',
  'menu.pending-join-requests': 'Permintaan Menunggu',
  'menu.dashboard': 'Dasbor',
  'menu.dashboard.analysis': 'Analisis',
  'menu.dashboard.monitor': 'Monitor',
  'menu.dashboard.workplace': 'Workplace',
  'menu.exception.403': '403',
  'menu.exception.404': '404',
  'menu.exception.500': '500',
  'menu.form': 'Form',
  'menu.form.basic-form': 'Form Dasar',
  'menu.form.step-form': 'Form Bertahap',
  'menu.form.step-form.info': 'Form Bertahap(menulis informasi yang dibagikan)',
  'menu.form.step-form.confirm':
    'Form Bertahap(konfirmasi informasi yang dibagikan)',
  'menu.form.step-form.result': 'Form Bertahap(selesai)',
  'menu.form.advanced-form': 'Form Lanjutan',
  'menu.list': 'Daftar',
  'menu.list.table-list': 'Tabel Pencarian',
  'menu.list.basic-list': 'Daftar Dasar',
  'menu.list.card-list': 'Daftar Kartu',
  'menu.list.search-list': 'Daftar Pencarian',
  'menu.list.search-list.articles': 'Daftar Pencarian(artikel)',
  'menu.list.search-list.projects': 'Daftar Pencarian(projek)',
  'menu.list.search-list.applications': 'Daftar Pencarian(aplikasi)',
  'menu.profile': 'Profil',
  'menu.profile.basic': 'Profil Dasar',
  'menu.profile.advanced': 'Profile Lanjutan',
  'menu.result': 'Hasil',
  'menu.result.success': 'Sukses',
  'menu.result.fail': 'Gagal',
  'menu.exception': 'Pengecualian',
  'menu.exception.not-permission': '403',
  'menu.exception.not-find': '404',
  'menu.exception.server-error': '500',
  'menu.exception.trigger': 'Jalankan',
  'menu.account': 'Akun',
  'menu.account.center': 'Detail Akun',
  'menu.account.changePassword': 'Ubah Kata Sandi',
  'menu.account.settings': 'Pengaturan Akun',
  'menu.account.help': 'Bantuan',
  'menu.account.trigger': 'Mengaktivasi Error',
  'menu.account.logout': 'Keluar',
  'menu.account.company': 'Ganti Perusahaan',
  'menu.account.language': 'Ganti Bahasa',
  // Manajemen Alur Kerja
  'menu.workflow': 'Manajemen Alur Kerja',
  'menu.workflow.list': 'Definisi Alur Kerja',
  'menu.workflow.list.desc': 'Mengelola dan mengonfigurasi definisi alur kerja',
  'menu.workflow.create': 'Buat Alur Kerja',
  'menu.workflow.create.desc': 'Buat definisi alur kerja baru',
  'menu.workflow.monitor': 'Monitor Alur Kerja',
  'menu.workflow.monitor.desc': 'Memantau eksekusi instansi alur kerja',
  // Manajemen Dokumen
  'menu.document': 'Manajemen Dokumen',
  'menu.document.list': 'Daftar Dokumen',
  'menu.document.list.desc': 'Melihat dan mengelola dokumen',
  'menu.document.create': 'Buat Dokumen',
  'menu.document.create.desc': 'Buat dokumen baru',
  'menu.document.approval': 'Persetujuan Saya',
  'menu.document.approval.desc': 'Memproses persetujuan dokumen yang tertunda',
  'menu.editor': 'Penyusun Grafis',
  'menu.editor.flow': 'Penyusun Alur',
  'menu.editor.mind': 'Penyusun Mind',
  'menu.editor.koni': 'Penyusun Koni',
};
