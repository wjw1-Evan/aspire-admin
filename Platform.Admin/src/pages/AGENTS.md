...
## WHERE TO LOOK
- **API 服务**：如需抽离 API 层，可存放在 `@/services` 目录。简单模块推荐直接内联在页面组件中（参考 `password-book` 模式）。
...
- **接口调用**：简单模块 API 直接内联在页面组件中（参考 `password-book`），复杂模块可抽离到 `@/services`。统一使用 `request`（来自 `@umijs/max`）发起请求，响应类型使用 `ApiResponse<T>`。
