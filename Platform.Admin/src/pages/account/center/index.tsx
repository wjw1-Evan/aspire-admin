...
const UserCenter: React.FC = () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const profile: UserProfile = {
          ...
          role: apiUser.access || (apiUser.roles?.includes('管理员') ? 'admin' : 'user'),
      ...
      setInitialState((s) => {
        ...
        return {
          ...
          currentUser: {
            ...s.currentUser,
            ...updateData,
            displayName: updateData.displayName || s.currentUser.displayName,
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      message.error(intl.formatMessage({ id: 'pages.account.center.avatar.sizeTooLarge' }));
      e.target.value = '';
      return;
    }
    if (!file.type.startsWith('image/')) {
      message.error(intl.formatMessage({ id: 'pages.account.center.avatar.invalidType' }));
      e.target.value = '';
      return;
    }
  ...
  if (loading) {
    return (
      <div>
        <FormattedMessage id="pages.account.center.loading" />
      </div>
    );
  }
  ...
  if (!userProfile) {
    return (
      <div>
        <FormattedMessage id="pages.account.center.userNotExist" />
      </div>
    );
  }
    ...
    <>
      ...
      <div className={styles.container}>
        <Title level={2}>
          ...
          <FormattedMessage id="pages.account.center.title" />
        </Title>

        <Card
          title={<FormattedMessage id="pages.account.center.profile" />}
          className={`${commonStyles.card} ${styles.profileCard}`}
          extra={
            <Button type="primary" icon={<EditOutlined />} onClick={handleOpenEditModal}>
              <FormattedMessage id="pages.account.center.editProfile" />
            </Button>
          }
        >
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <Tag color={getRoleTagColor(userProfile.role)}>
                {userProfile.role === 'admin' ? (
                  <FormattedMessage id="pages.account.center.admin" />
                ) : (
                  <FormattedMessage id="pages.account.center.user" />
                )}
              </Tag>
            </div>
          </div>
          ...
          <ProDescriptions column={2} bordered>
            ...
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.name" />}>
              {userProfile.name || <FormattedMessage id="pages.account.center.notSet" />}
            </ProDescriptions.Item>
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.email" />}>
              <MailOutlined style={{ marginRight: '4px' }} />
              {userProfile.email || <FormattedMessage id="pages.account.center.notSet" />}
            </ProDescriptions.Item>
            <ProDescriptions.Item
              label={
                <Space>
                  <MobileOutlined />
                  <FormattedMessage id="pages.account.center.phoneNumber" />
                </Space>
              }
            >
              {userProfile.phoneNumber || <FormattedMessage id="pages.account.center.notSet" />}
            </ProDescriptions.Item>
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.age" />}>
              {userProfile.age || <FormattedMessage id="pages.account.center.notSet" />}
            </ProDescriptions.Item>
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.role" />}>
              <Tag color={getRoleTagColor(userProfile.role)}>
                {userProfile.role === 'admin' ? (
                  <FormattedMessage id="pages.account.center.admin" />
                ) : (
            ...
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.status" />}>
              <Tag color={userProfile.isActive ? 'green' : 'red'}>
                {userProfile.isActive ? (
                  <FormattedMessage id="pages.account.center.active" />
                ) : (
                  <FormattedMessage id="pages.account.center.inactive" />
                )}
              </Tag>
            </ProDescriptions.Item>
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.registerTime" />}>
              <CalendarOutlined style={{ marginRight: '4px' }} />
              {userProfile.createdAt ? dayjs(userProfile.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
            </ProDescriptions.Item>
            <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.lastUpdate" />}>
              <CalendarOutlined style={{ marginRight: '4px' }} />
              {userProfile.updatedAt ? dayjs(userProfile.updatedAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
            </ProDescriptions.Item>
            {userProfile.lastLoginAt && (
              <ProDescriptions.Item label={<FormattedMessage id="pages.account.center.lastLogin" />}>
                <CalendarOutlined style={{ marginRight: '4px' }} />
        ...
        >
            ...
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              {(typeof avatarPreview === 'string' ? avatarPreview : userProfile?.avatar) && (
                <Button
                  ...
                  style={{ position: 'absolute', top: -8, right: -8, minWidth: 24, height: 24, padding: 0, zIndex: 10 }}
                  title={intl.formatMessage({ id: 'pages.account.center.avatar.delete' })}
                />
              )}
            </div>
          </div>
          <Form form={form} onFinish={handleUpdateProfile} layout="vertical">
            <Form.Item
              ...
              label={
                <span>
                  <FormattedMessage id="pages.account.center.username" />
                  <Tooltip title={intl.formatMessage({ id: 'pages.account.center.username.notEditable' })}>
            ...
            <Form.Item
              name="email"
              label={<FormattedMessage id="pages.account.center.email" />}
              rules={[{ type: 'email', message: intl.formatMessage({ id: 'pages.account.center.email.invalid' }) }]}
            >
              <Input />
            </Form.Item>
            <Form.Item
              name="phoneNumber"
              label={intl.formatMessage({ id: 'pages.account.center.phoneNumber' })}
              rules={[
                {
                  pattern: /^1[3-9]\d{9}$/,
                  message: intl.formatMessage({ id: 'pages.account.center.phoneNumber.invalid' }),
                },
              ]}
            >
              <Input
                placeholder={intl.formatMessage({ id: 'pages.account.center.phoneNumber.placeholder' })}
                maxLength={11}
              />
            </Form.Item>
            <Form.Item
              name="age"
              label={<FormattedMessage id="pages.account.center.age" />}
              rules={[
                {
                  type: 'number',
                  min: 1,
                  max: 150,
                  message: intl.formatMessage({ id: 'pages.account.center.age.invalid' }),
                },
              ]}
            >
              <InputNumber
                min={1}
                max={150}
                placeholder={intl.formatMessage({ id: 'pages.account.center.age.placeholder' })}
                style={{ width: '100%' }}
              />
            </Form.Item>
              ...
              <div style={{ textAlign: 'right' }}>
                <Space>
                  <Button onClick={handleCancelEdit}>
                    <FormattedMessage id="pages.account.center.cancel" />
                  </Button>
                  <Button type="primary" htmlType="submit">
                    <FormattedMessage id="pages.account.center.save" />
                  </Button>
                </Space>
              </div>
            </Form.Item>
        ...
        >
          ...
          <Form form={changePasswordForm} onFinish={handleChangePassword} layout="vertical">
            <Form.Item
              ...
              rules={[
                { required: true, message: <FormattedMessage id="pages.changePassword.currentPassword.required" /> },
              ]}
            >
              <Input.Password
                prefix={<LockOutlined />}
                ...
                ({ getFieldValue }) => ({
                  validator(_, value) {
                    ...
                    return Promise.reject(
                      new Error(intl.formatMessage({ id: 'pages.changePassword.confirmPassword.mismatch' })),
                    );
                  },
                }),
              ]}
