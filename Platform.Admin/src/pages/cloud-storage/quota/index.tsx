...
interface UserQuotaResponse { userId: string; totalQuota: number; usedSpace: number; }
...
const CloudStorageQuotaPage: React.FC = () => {
    ...
    const columns: ProColumns<StorageQuota>[] = [
        { title: intl.formatMessage({ id: 'pages.cloud-storage.quota.field.user' }), dataIndex: 'displayName', key: 'displayName', sorter: true, render: (dom: any, record: StorageQuota) => (<Space><UserOutlined /><a onClick={() => handleView(record)} style={{ cursor: 'pointer' }} title={dom || record.username || record.userId}>{dom || record.username || record.userId || intl.formatMessage({ id: 'pages.table.unknown' })}</a></Space>) },
        ...
        { title: intl.formatMessage({ id: 'pages.cloud-storage.quota.field.usedSpace' }), dataIndex: 'usedSpace', key: 'usedSpace', sorter: true, render: (dom: any) => formatFileSize(dom) },
                ...
                <Tabs activeKey={state.activeTab} onChange={handleTabChange} items={[
                    ...
                    { key: 'usage-stats', label: <Space><PieChartOutlined />{intl.formatMessage({ id: 'pages.cloud-storage.quota.tabs.usageStats' })}</Space>, children: state.usageStats ? (<Space orientation="vertical" style={{ width: '100%' }} size="large">
                        <Row gutter={[16, 16]}>
                         ...
                         <Row gutter={[16, 16]}>
                             <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.cloud-storage.quota.usageDistribution.title' })}><div style={{ height: 350, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              ...
                              <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.cloud-storage.quota.usageRanking.title' })}><div style={{ height: 350, overflowY: 'auto' }}>{state.usageStats.topUsers.map((user, i) => (<div key={user.userId} style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><Space><Typography.Text strong>{i + 1}.</Typography.Text><Typography.Text>{user.displayName || user.username}</Typography.Text></Space><Typography.Text type="secondary">{formatFileSize(user.usedSpace)} / {formatFileSize(user.usagePercentage > 0 ? user.usedSpace / (user.usagePercentage / 100) : 0)}</Typography.Text></div><Progress percent={parseFloat(fmtPct(user.usagePercentage))} size="small" status={Number(user.usagePercentage) > 90 ? 'exception' : 'active'} /></div>))}</div></ProCard></Col>
                        </Row>
            ...
            <Drawer title={intl.formatMessage({ id: 'pages.cloud-storage.quota.drawer.title' })} placement="right" onClose={() => set({ detailVisible: false })} open={state.detailVisible} size="large">
                <Spin spinning={!state.viewingQuota}>
                    {state.viewingQuota && (<ProCard title={intl.formatMessage({ id: 'pages.cloud-storage.quota.drawer.basicInfo' })} className={styles.card} style={{ marginBottom: 16 }}>
                        <Row gutter={[16, 16]}>
                            <Col span={24}><Space><UserOutlined />{intl.formatMessage({ id: 'pages.cloud-storage.quota.field.user' })}: </Space>{state.viewingQuota.displayName && state.viewingQuota.displayName !== state.viewingQuota.username ? `${state.viewingQuota.displayName} (${state.viewingQuota.username})` : state.viewingQuota.displayName || state.viewingQuota.username || '-'}</Col>
                            ...
                            <Col xs={24} sm={12}><Space><BarChartOutlined />{intl.formatMessage({ id: 'pages.cloud-storage.quota.field.usedSpace' })}: </Space>{formatFileSize(state.viewingQuota.usedSpace)}</Col>
                            <Col xs={24} sm={12}><Space><PieChartOutlined />{intl.formatMessage({ id: 'pages.cloud-storage.quota.field.usagePercentage' })}: </Space><Progress percent={state.viewingQuota.totalQuota > 0 ? Math.round((state.viewingQuota.usedSpace / state.viewingQuota.totalQuota) * 100) : 0} size="small" strokeColor={getUsageColor(state.viewingQuota.totalQuota > 0 ? (state.viewingQuota.usedSpace / state.viewingQuota.totalQuota) * 100 : 0)} /></Col>
