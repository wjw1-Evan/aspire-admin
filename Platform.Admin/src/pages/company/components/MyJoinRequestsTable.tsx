        ...
        if (record.status === 'rejected') {
          return (
            <span style={{ color: token.colorError }}>
              {intl.formatMessage({ id: 'pages.company.joinRequest.rejectReason' })}: {dom}
            </span>
          );
        }
        return '-';
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.status === 'pending') {
          return (
            <Popconfirm
              title={intl.formatMessage({ id: 'pages.company.joinRequest.confirmCancel' })}
            ...
            >
              <Button type="link" size="small" danger icon={<UndoOutlined />}>
                {intl.formatMessage({ id: 'pages.common.revoke' })}
              </Button>
            </Popconfirm>
          );
        }
    ...
    <ProTable<JoinRequestDetail>
      headerTitle={
        <Space size={24}>
          <Space>
            <TeamOutlined />
            {intl.formatMessage({ id: 'pages.company.myRequests.title' })}
          </Space>
        </Space>
      }
      rowKey="id"
