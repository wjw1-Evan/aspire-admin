...
import { Badge, Progress, Space, Spin, Tag, Typography } from 'antd';
...
const StatisticCard: React.FC<{ style: StyleConfig; value?: unknown }> = ({ style, value }) => {
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%', padding: '8px 4px' }}
    >
      {icon && (
...
const GaugeChart: React.FC<{ style: StyleConfig; width?: number; height?: number }> = ({ style, width, height }) => {
  ...
  return (
    <ReactECharts
      ...
      opts={{ width: width || undefined, height: height || undefined, renderer: 'svg' }}
    />
  );
};
              ...
              children: [
                {
                  ...
                  style: {
                    ...
                    fontWeight: 700,
                    textAlign: 'center',
                    x: showLegend && legendPosition === 'right' ? -60 : 0,
                  },
                },
                {
                  type: 'text',
...
/** 折线图/面积图 */
const LineAreaChart: React.FC<{ style: StyleConfig; isArea?: boolean; width?: number; height?: number }> = ({
  style,
  isArea = false,
  width,
  height,
}) => {
  const { xAxisData = [], seriesData = [], showLegend = true, smooth = true, gridTop = 40, gridBottom = 30 } = style;
...
const BarChart: React.FC<{ style: StyleConfig; width?: number; height?: number }> = ({ style, width, height }) => {
  const { xAxisData = [], seriesData = [], showLegend = true, gridTop = 40, gridBottom = 30 } = style;
  const defaultColors = ['#00d4ff', '#36cbcb', '#4ecb73', '#fbd437', '#f2637b'];
          ...
          >
            ...
            {item.data &&
              Object.entries(item.data).map(([k, v]) => (
                ...
                >
                  <span>{k}</span>
                  <span style={{ color: DARK_THEME.text }}>{v}</span>
                </div>
              ))}
            <Tag
              color={STATUS_COLORS[item.status]}
              style={{ marginTop: 4, borderRadius: 4, fontSize: 11, lineHeight: '18px' }}
            >
      ...
      {modules.map(
        (
          ...
          >
            ...
            {mod.description && (
              <Text style={{ color: DARK_THEME.subText, fontSize: 10, textAlign: 'center' }}>{mod.description}</Text>
            )}
      ...
      {alerts.map((alert: { time: string; level: string; message: string }, idx: number) => (
        ...
        >
          <Tag
            ...
            style={{ minWidth: 50, textAlign: 'center', borderRadius: 4, fontSize: 12, lineHeight: '22px' }}
          >
            {alert.time}
          </Tag>
...
const HeaderCard: React.FC<{ style: StyleConfig }> = ({ style }) => {
  const { headerTitle = '数据看板', headerIcon = '', textColor = '#fff', fontSize = 22, textAlign = 'center' } = style;
...
const ClockCard: React.FC<{ style: StyleConfig }> = ({ style }) => {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', height: '100%', padding: '0 16px' }}
    >
      <Text style={{ color: textColor, fontSize, fontFamily: "'DIN Alternate', 'Roboto Mono', monospace" }}>
        {dateStr}
      </Text>
    </div>
  );
    ...
    >
      {statusItems.map((item: { icon?: string; text: string; color?: string }, idx: number) => (
        <Space key={idx} size={6}>
          {item.icon === 'success' ? (
            <CheckCircleFilled style={{ color: item.color || '#52c41a' }} />
          ) : item.icon === 'warning' ? (
            <WarningFilled style={{ color: item.color || '#faad14' }} />
          ) : item.icon === 'error' ? (
            <CloseCircleFilled style={{ color: item.color || '#ff4d4f' }} />
          ) : (
...
const ImageCard: React.FC<{ style: StyleConfig }> = ({ style }) => {
  ...
  if (!imageUrl)
    return <div style={{ color: DARK_THEME.subText, textAlign: 'center', paddingTop: 20 }}>请配置图片地址</div>;
  return (
...
const CardRenderer: React.FC<CardRendererProps> = ({ card, width, height }) => {
  ...
  const renderContent = () => {
    ...
    switch (card.cardType) {
      ...
      default:
        return (
          <div style={{ color: DARK_THEME.subText, textAlign: 'center', paddingTop: 16 }}>
  ...
  return (
    ...
    >
      {showTitle && (
        ...
        >
          {styleConfig.icon && (
            <span style={{ color: styleConfig.iconColor || '#00d4ff' }}>{ICON_MAP[styleConfig.icon] || ''}</span>
          )}
          {card.title}
        </div>
      )}
