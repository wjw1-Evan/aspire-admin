...
const api = {
  ...
  create: (data: Partial<Document>) =>
    request<ApiResponse<Document>>('/apiservice/api/documents', { method: 'POST', data }),
...
const DocumentManagement: React.FC = () => {
  ...
  const columns: ProColumns<Document>[] = [
    {
      title: intl.formatMessage({ id: 'pages.document.columns.title' }),
      dataIndex: 'title',
      ellipsis: true,
      sorter: true,
      render: (dom: any, r) => (
        <a onClick={() => set({ viewingId: r.id, detailVisible: true })}>
          <FileTextOutlined /> {dom}
        </a>
      ),
    },
    {
      title: intl.formatMessage({ id: 'pages.document.columns.type' }),
      dataIndex: 'documentType',
      ellipsis: true,
      sorter: true,
    },
    {
      title: intl.formatMessage({ id: 'pages.document.columns.category' }),
      dataIndex: 'category',
      ellipsis: true,
      sorter: true,
      render: (dom: any) => (dom ? <Tag color="blue">{dom}</Tag> : '-'),
    },
    {
      title: intl.formatMessage({ id: 'pages.document.columns.status' }),
      dataIndex: 'status',
      sorter: true,
      render: (dom: any) => {
        const status = documentStatusMap[dom as DocumentStatus] || {
          textId: 'pages.document.status.unknown',
          defaultText: '未知',
          color: 'default',
        };
    ...
    {
      title: intl.formatMessage({ id: 'pages.document.columns.createdBy' }),
      dataIndex: 'createdBy',
      ellipsis: true,
      sorter: true,
    },
    {
      title: intl.formatMessage({ id: 'pages.document.columns.createdAt' }),
      dataIndex: 'createdAt',
      sorter: true,
      valueType: 'dateTime',
    },
      ...
      render: (_: any, r: Document) => (
        <Space size={4}>
          {r.status === DocumentStatus.Draft && (
            <>
              <Button
                ...
                size="small"
                icon={<EditOutlined />}
                onClick={() => set({ editVisible: true, editingId: r.id! })}
              >
              ...
              <Button
                ...
                size="small"
                icon={<SendOutlined />}
                onClick={() => set({ submitVisible: true, submittingId: r.id! })}
              >
                {intl.formatMessage({ id: 'pages.document.submit' })}
              </Button>
            </>
          )}
          {r.status === DocumentStatus.Approved && (
            ...
            >
              {intl.formatMessage({ id: 'pages.document.action.archive' })}
            </Button>
          )}
          <Popconfirm
            title={intl.formatMessage({ id: 'pages.document.deleteConfirm' }, { title: r.title })}
            onConfirm={async () => {
          ...
          >
            <Button type="link" size="small" danger icon={<DeleteOutlined />}>
              {intl.formatMessage({ id: 'pages.document.delete' })}
            </Button>
          </Popconfirm>
        </Space>
      ),
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        headerTitle={
          <Space size={24}>
            ...
            <Space size={12}>
              <Tag color="blue">
                {intl.formatMessage({ id: 'pages.document.statistics.total' })} {state.statistics?.totalDocuments || 0}
              </Tag>
              <Tag color="default">
                {intl.formatMessage({ id: 'pages.document.statistics.draft' })} {state.statistics?.draftCount || 0}
              </Tag>
              <Tag color="processing">
                {intl.formatMessage({ id: 'pages.document.statistics.pending' })} {state.statistics?.pendingCount || 0}
              </Tag>
              <Tag color="success">
                {intl.formatMessage({ id: 'pages.document.statistics.approved' })}{' '}
                {state.statistics?.approvedCount || 0}
              </Tag>
              <Tag color="warning">
                {intl.formatMessage({ id: 'pages.document.statistics.archived' })}{' '}
                {state.statistics?.archivedCount || 0}
              </Tag>
              <Tag color="error">
                {intl.formatMessage({ id: 'pages.document.statistics.rejected' })}{' '}
                {state.statistics?.rejectedCount || 0}
              </Tag>
            </Space>
          </Space>
        }
        ...
        toolBarRender={() => [
          ...
          />,
          <Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => set({ formVisible: true })}>
            {intl.formatMessage({ id: 'pages.document.createDocument' })}
          </Button>,
        ]}
      />
      ...
      <Drawer
        title={intl.formatMessage({ id: 'pages.document.detail' })}
        placement="right"
      ...
      <ModalForm
        ...
        onOpenChange={(open) => {
          if (!open) set({ submitVisible: false, submittingId: '' });
        }}
        onFinish={async (values) => {
...
const DocumentDetail: React.FC<{ id: string }> = ({ id }) => {
  ...
  useEffect(() => {
    if (id)
      api.get(id).then((r) => {
        if (r.success && r.data) setDoc(r.data);
      });
  }, [id]);
  if (!doc) return null;
  return (
    <ProDescriptions column={1} bordered size="small">
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.title' })}>
        <strong>{doc.title}</strong>
      </ProDescriptions.Item>
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.type' })}>
        {doc.documentType}
      </ProDescriptions.Item>
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.category' })}>
      ...
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.status' })}>
        <Tag color={documentStatusMap[doc.status]?.color || 'default'}>
          {intl.formatMessage({ id: documentStatusMap[doc.status]?.textId })}
        </Tag>
      </ProDescriptions.Item>
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.createdBy' })}>
        {doc.createdBy}
      </ProDescriptions.Item>
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.createdAt' })}>
        {dayjs(doc.createdAt).format('YYYY-MM-DD HH:mm')}
      </ProDescriptions.Item>
      <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.updatedAt' })}>
        {dayjs(doc.updatedAt).format('YYYY-MM-DD HH:mm')}
      </ProDescriptions.Item>
      {doc.content && (
        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.document.columns.content' })} span={1}>
...
const DocumentEditDrawer: React.FC<{ id: string; visible: boolean; onClose: () => void; onSuccess: () => void }> = ({
  id,
  visible,
  onClose,
  onSuccess,
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        ...
        />
        <ProFormTextArea name="content" label={intl.formatMessage({ id: 'pages.document.form.content' })} rows={6} />
        <ProFormText name="documentType" label={intl.formatMessage({ id: 'pages.document.form.documentType' })} />
        <ProFormText name="category" label={intl.formatMessage({ id: 'pages.document.form.category' })} />
        <Form.Item>
          <Space>
            <Button type="primary" htmlType="submit" loading={loading}>
              {intl.formatMessage({ id: 'pages.document.save' })}
            </Button>
