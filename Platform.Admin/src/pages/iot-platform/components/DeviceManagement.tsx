...
const DeviceManagement = React.forwardRef<DeviceManagementRef, any>((props, ref) => {
  ...
  const columns: ProColumns<IoTDevice>[] = [
    ...
    { title: intl.formatMessage({ id: 'pages.iotPlatform.table.action' }), valueType: 'option', fixed: 'right', width: 180, render: (_, record) => (
      <Space size={4}>
        <Button type="link" size="small" icon={<EditOutlined />} onClick={() => { set({ editingDevice: record, formVisible: true }); form.setFieldsValue({ ...record, tagsList: record.tags ? Object.entries(record.tags).map(([k, v]) => `${k}:${v}`) : [] }); }}>{intl.formatMessage({ id: 'pages.iotPlatform.device.edit' })}</Button>
