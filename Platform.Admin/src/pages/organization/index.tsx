...
const api = {
  ...
  create: (data: CreateOrgRequest) =>
    request<ApiResponse<OrgNode>>('/apiservice/api/organization', { method: 'POST', data }),
  update: (id: string, data: UpdateOrgRequest) =>
    request<ApiResponse<boolean>>(`/apiservice/api/organization/${id}`, { method: 'PUT', data }),
  delete: (id: string) => request<ApiResponse<boolean>>(`/apiservice/api/organization/${id}`, { method: 'DELETE' }),
  reorder: (items: ReorderItem[]) =>
    request<ApiResponse<boolean>>('/apiservice/api/organization/reorder', { method: 'POST', data: items }),
  ...
  assignUser: (data: AssignUserRequest) =>
    request<ApiResponse<boolean>>('/apiservice/api/organization/assign-user', { method: 'POST', data }),
  removeUser: (data: { userId: string; organizationUnitId: string }) =>
    request<ApiResponse<boolean>>('/apiservice/api/organization/remove-user', { method: 'POST', data }),
  availableUsers: (orgId: string, query?: string) =>
    request<ApiResponse<AvailableUser[]>>('/apiservice/api/organization/available-users', {
      params: { organizationUnitId: orgId, ...(query ? { query } : {}) },
    }),
};
...
const buildTreeData = (nodes: OrgNode[], search: string): DataNode[] =>
  nodes.map((node) => {
    ...
    const title =
      idx > -1 ? (
        <span>
          ...
          <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>{name.substring(idx, idx + search.length)}</span>
          {name.substring(idx + search.length)}
        </span>
      ) : (
        <span>{name}</span>
      );
    return {
      ...
      children: node.children ? buildTreeData(node.children, search) : undefined,
    };
  });
...
const AssignUserModal: React.FC<{
  ...
  const handleSearch = (value: string) => {
    ...
    setLoading(true);
    api
      .availableUsers(orgId, value || undefined)
      .then((r) => {
        if (r.success && r.data) setUsers(r.data);
      })
      .finally(() => setLoading(false));
  ...
  return (
    <Modal
      open={open}
      title={intl.formatMessage({ id: 'pages.organization.modal.assignUser' })}
      destroyOnHidden
      onCancel={onCancel}
      onOk={async () => {
    ...
    >
      <Form form={form} layout="vertical">
        ...
        <Form.Item
          name="userId"
          label={intl.formatMessage({ id: 'pages.organization.form.user' })}
          rules={[{ required: true, message: intl.formatMessage({ id: 'pages.organization.form.userRequired' }) }]}
        >
          <Select
            showSearch
            placeholder={intl.formatMessage({ id: 'pages.organization.form.searchUser' })}
            loading={loading}
            options={users}
...
const OrganizationPage: React.FC = () => {
  ...
  const [state, setState] = useState({
    ...
    formOpen: false,
    submitLoading: false,
    editingNode: null as OrgNode | null,
  ...
  const selectedNode = useMemo(
    () => (state.selectedId ? nodeMap[state.selectedId] || null : null),
    [nodeMap, state.selectedId],
  );
  ...
  const fetchMembers = useCallback(
    async (orgId?: string) => {
      ...
      if (res.success && res.data) set({ members: res.data });
      else set({ members: [] });
    },
    [set],
  );
  ...
  const buildParentChildrenMap = useCallback(() => {
    ...
    const dfs = (nodes: OrgNode[], parentId?: string) =>
      nodes.forEach((n) => {
        const id = n.id!;
  ...
  const handleDrop = async (info: any) => {
    ...
    if (newParentId === dragId || (newParentId && dragDescendants.has(newParentId))) {
      message.error(intl.formatMessage({ id: 'pages.organization.message.cannotMove' }));
      return;
    }
    ...
    const baseTarget =
      dragNode.parentId === newParentId ? [...targetSiblings].filter((id) => id !== dragId) : [...targetSiblings];
    ...
    if (dragNode.parentId !== newParentId)
      filteredSource.forEach((id, idx) => items.push({ id, parentId: dragNode.parentId, sortOrder: idx + 1 }));
  ...
  return (
    <PageContainer>
            ...
            <Spin spinning={state.loading}>
              {treeData.length > 0 ? (
                <Tree
                  ...
                  draggable
                  onDrop={handleDrop}
                  selectedKeys={state.selectedId ? [state.selectedId] : []}
                  onSelect={(keys) => {
                    if (keys[0]) set({ selectedId: keys[0] as string });
                  }}
                  treeData={treeData}
                />
              ) : (
        ...
        <Col xs={24} md={16} lg={17}>
          <Card
            ...
            extra={
              selectedNode ? (
                <Space wrap>
                  <Button icon={<PlusOutlined />} onClick={() => openCreateModal(selectedNode.id)}>
                    {intl.formatMessage({ id: 'pages.organization.action.createChild' })}
                  </Button>
                  <Button icon={<EditOutlined />} onClick={() => set({ editingNode: selectedNode, formOpen: true })}>
            ...
            {selectedNode ? (
              <>
                <ProDescriptions column={2} bordered>
                  ...
                  <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.organization.field.code' })}>
                    {selectedNode.code ? <Tag color="blue">{selectedNode.code}</Tag> : '-'}
                  </ProDescriptions.Item>
                  <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.organization.field.parent' })}>
                    {selectedNode.parentId ? nodeMap[selectedNode.parentId]?.name || '-' : '-'}
                  </ProDescriptions.Item>
                  <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.organization.field.sortOrder' })}>
                    <Tag color="purple">{selectedNode.sortOrder ?? 1}</Tag>
                  </ProDescriptions.Item>
                  <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.organization.field.manager' })}>
                  ...
                  <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.organization.field.createdAt' })}>
                    {selectedNode.createdAt ? dayjs(selectedNode.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-'}
                  </ProDescriptions.Item>
                </ProDescriptions>
                <Card
                  className={styles.card}
                ...
                >
                  {state.members.length ? (
                      ...
                      {state.members.map((m, index) => (
                        ...
                        >
                          <Space>
                            ...
                            {m.email && <Tag color="geekblue">{m.email}</Tag>}
                          </Space>
                          <Button
                            type="link"
                            danger
                            ...
                            onClick={() => {
                              Modal.confirm({
                                title: intl.formatMessage({ id: 'pages.organization.modal.confirmRemove' }),
                                content: intl.formatMessage(
                                  { id: 'pages.organization.modal.removeContent' },
                          ...
                          >
                            {intl.formatMessage({ id: 'pages.organization.members.remove' })}
                          </Button>
                        </div>
                      ))}
                    </div>
                    ...
                    <Empty
                      image={Empty.PRESENTED_IMAGE_SIMPLE}
                      description={intl.formatMessage({ id: 'pages.organization.members.empty' })}
                    />
                  )}
                </Card>
              </>
      ...
      <ModalForm
        key={state.editingNode?.id || 'create'}
        title={
          state.editingNode
            ? intl.formatMessage({ id: 'pages.organization.modal.updateTitle' })
            : intl.formatMessage({ id: 'pages.organization.modal.createTitle' })
        }
        open={state.formOpen}
        onOpenChange={(open) => {
          if (!open) set({ formOpen: false, editingNode: null });
        }}
        initialValues={
          state.editingNode
            ? {
                name: state.editingNode.name,
                code: state.editingNode.code,
                parentId: state.editingNode.parentId,
                description: state.editingNode.description,
                sortOrder: state.editingNode.sortOrder ?? 1,
                managerUserId: state.editingNode.managerUserId,
              }
            : { parentId: state.createParentId, sortOrder: 1 }
        }
        onFinish={async (values) => {
          set({ submitLoading: true });
          try {
            if (state.editingNode?.id) {
              await api.update(state.editingNode.id, values as UpdateOrgRequest);
              message.success(intl.formatMessage({ id: 'pages.message.updateSuccess' }));
            } else {
              const res = await api.create(values as CreateOrgRequest);
              message.success(intl.formatMessage({ id: 'pages.message.createSuccess' }));
              if (res.success && res.data?.id) set({ selectedId: res.data.id });
            }
      ...
      >
        ...
        />
        <Form.Item name="parentId" label={intl.formatMessage({ id: 'pages.organization.field.parent' })}>
          <TreeSelect
            allowClear
        ...
        <ProFormDigit
          ...
          min={1}
          placeholder={intl.formatMessage({ id: 'pages.organization.placeholder.sortOrder' })}
