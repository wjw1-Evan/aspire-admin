...
const AssetManagement: React.FC = () => {
    ...
    const renderBuildingType = (buildingType?: string) => {
        const typeMap: Record<string, { color: string; text: string }> = {
            Office: { color: 'blue', text: intl.formatMessage({ id: 'pages.park.asset.buildingType.office', defaultMessage: '办公楼' }) },
            Commercial: { color: 'green', text: intl.formatMessage({ id: 'pages.park.asset.buildingType.commercial', defaultMessage: '商业楼' }) },
            Mixed: { color: 'orange', text: intl.formatMessage({ id: 'pages.park.asset.buildingType.mixed', defaultMessage: '综合楼' }) },
        };
    ...
    const renderBuildingStatus = (status: string) => {
        const statusMap: Record<string, { color: string; text: string }> = {
            Active: { color: 'green', text: intl.formatMessage({ id: 'pages.park.asset.status.active', defaultMessage: '正常' }) },
            Maintenance: { color: 'orange', text: intl.formatMessage({ id: 'pages.park.asset.status.maintenance', defaultMessage: '维护中' }) },
            Inactive: { color: 'default', text: intl.formatMessage({ id: 'pages.park.asset.status.inactive', defaultMessage: '停用' }) },
        };
    ...
    const handleDeleteBuilding = async (id: string) => {
        ...
        if (res.success) {
            message.success(intl.formatMessage({ id: 'pages.park.asset.deleteSuccess', defaultMessage: '删除成功' }));
    ...
    const buildingColumns: ProColumns<Building>[] = [
        ...
        { title: intl.formatMessage({ id: 'pages.park.asset.building.type', defaultMessage: '类型' }), dataIndex: 'buildingType', sorter: true, width: 100, render: (_, record) => renderBuildingType(record.buildingType) },
        ...
        { title: intl.formatMessage({ id: 'pages.park.asset.building.status', defaultMessage: '状态' }), dataIndex: 'status', sorter: true, width: 100, render: (_, record) => renderBuildingStatus(record.status) },
        { title: intl.formatMessage({ id: 'common.action', defaultMessage: '操作' }), valueType: 'option', fixed: 'right', width: 180, render: (_, record) => (
            <Space size={4}>
                ...
                <Button type="link" size="small" icon={<EditOutlined />} onClick={() => { setBuilding({ editingBuilding: record, modalVisible: true }); setForm({ attachments: mapAttachments(record.attachments) }); }}>{intl.formatMessage({ id: 'common.edit', defaultMessage: '编辑' })}</Button>
                <Popconfirm title={intl.formatMessage({ id: 'common.confirmDelete', defaultMessage: '确认删除？' })} onConfirm={() => handleDeleteBuilding(record.id)}>
    ...
    const renderUnitType = (unitType?: string) => {
        const typeMap: Record<string, { color: string; text: string }> = {
            Office: { color: 'blue', text: intl.formatMessage({ id: 'pages.park.asset.unitType.office', defaultMessage: '办公' }) },
            Commercial: { color: 'green', text: intl.formatMessage({ id: 'pages.park.asset.unitType.commercial', defaultMessage: '商铺' }) },
            Warehouse: { color: 'purple', text: intl.formatMessage({ id: 'pages.park.asset.unitType.warehouse', defaultMessage: '仓储' }) },
        };
        const config = typeMap[unitType || ''] || { color: 'default', text: unitType || intl.formatMessage({ id: 'pages.park.asset.unitType.other', defaultMessage: '其他' }) };
        return <Tag color={config.color}>{config.text}</Tag>;
    };

    const renderUnitStatus = (status: string) => {
        const statusMap: Record<string, { color: string; text: string }> = {
            Available: { color: 'green', text: intl.formatMessage({ id: 'pages.park.asset.unit.status.available', defaultMessage: '空置' }) },
            Rented: { color: 'blue', text: intl.formatMessage({ id: 'pages.park.asset.unit.status.rented', defaultMessage: '已出租' }) },
            Reserved: { color: 'orange', text: intl.formatMessage({ id: 'pages.park.asset.unit.status.reserved', defaultMessage: '预留' }) },
            Maintenance: { color: 'red', text: intl.formatMessage({ id: 'pages.park.asset.unit.status.maintenance', defaultMessage: '维护' }) },
        };
        const config = statusMap[status] || { color: 'default', text: status };
        return <Tag color={config.color}>{config.text}</Tag>;
    };
    ...
    const unitColumns: ProColumns<PropertyUnit>[] = [
        ...
        { title: intl.formatMessage({ id: 'pages.park.asset.unit.type', defaultMessage: '类型' }), dataIndex: 'unitType', sorter: true, width: 100, render: (_, record) => renderUnitType(record.unitType) },
        { title: intl.formatMessage({ id: 'pages.park.asset.unit.status', defaultMessage: '状态' }), dataIndex: 'status', sorter: true, width: 100, render: (_, record) => renderUnitStatus(record.status) },
        { title: intl.formatMessage({ id: 'common.action', defaultMessage: '操作' }), valueType: 'option', fixed: 'right', width: 180, render: (_, record) => (
            <Space size={4}>
                ...
                <Button type="link" size="small" icon={<EditOutlined />} onClick={() => { setUnit({ editingUnit: record, modalVisible: true }); setForm({ attachments: mapAttachments(record.attachments) }); }}>{intl.formatMessage({ id: 'common.edit', defaultMessage: '编辑' })}</Button>
                <Popconfirm title={intl.formatMessage({ id: 'common.confirmDelete', defaultMessage: '确认删除？' })} onConfirm={() => handleDeleteUnit(record.id)}>
                    <Button type="link" size="small" danger icon={<DeleteOutlined />}>{intl.formatMessage({ id: 'common.delete', defaultMessage: '删除' })}</Button>
                </Popconfirm>
            </Space>
        ) },
    ...
    return (
        <PageContainer>
            ...
            <ModalForm key={buildingState.editingBuilding?.id || 'create-building'}
                ...
                <ProFormSelect name="buildingType" label="楼宇类型" placeholder="请选择类型" options={[{ label: intl.formatMessage({ id: 'pages.park.asset.buildingType.office', defaultMessage: '办公楼' }), value: 'Office' }, { label: intl.formatMessage({ id: 'pages.park.asset.buildingType.commercial', defaultMessage: '商业楼' }), value: 'Commercial' }, { label: intl.formatMessage({ id: 'pages.park.asset.buildingType.mixed', defaultMessage: '综合楼' }), value: 'Mixed' }]} />
                ...
                <Row gutter={16}>
                    ...
                    <Col span={8}><ProFormSelect name="status" label="状态" placeholder="请选择状态" options={[{ label: intl.formatMessage({ id: 'pages.park.asset.status.active', defaultMessage: '正常' }), value: 'Active' }, { label: intl.formatMessage({ id: 'pages.park.asset.status.maintenance', defaultMessage: '维护中' }), value: 'Maintenance' }, { label: intl.formatMessage({ id: 'pages.park.asset.status.inactive', defaultMessage: '停用' }), value: 'Inactive' }]} /></Col>
                </Row>
            ...
            <ModalForm key={unitState.editingUnit?.id || 'create-unit'}
                ...
                <Row gutter={16}>
                    ...
                    <Col span={8}><ProFormSelect name="unitType" label="类型" placeholder="请选择类型" options={[{ label: intl.formatMessage({ id: 'pages.park.asset.unitType.office', defaultMessage: '办公' }), value: 'Office' }, { label: intl.formatMessage({ id: 'pages.park.asset.unitType.commercial', defaultMessage: '商铺' }), value: 'Commercial' }, { label: intl.formatMessage({ id: 'pages.park.asset.unitType.warehouse', defaultMessage: '仓储' }), value: 'Warehouse' }]} /></Col>
                </Row>
                <Row gutter={16}>
            ...
            <Drawer title={buildingState.currentBuilding?.name || intl.formatMessage({ id: 'pages.park.asset.buildingDetail', defaultMessage: '楼宇详情' })} open={buildingState.detailVisible} onClose={() => { setBuilding({ detailVisible: false, currentBuilding: null }); }} size="large" loading={buildingState.detailLoading}>
                {buildingState.currentBuilding && (<div>
                    <ProDescriptions bordered column={2} size="small" style={{ marginBottom: 24 }}>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.buildingName', defaultMessage: '楼宇名称' })}>{buildingState.currentBuilding.name}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.buildingType', defaultMessage: '楼宇类型' })}><Tag color="blue">{intl.formatMessage({ id: `pages.park.asset.buildingType.${buildingState.currentBuilding.buildingType?.toLowerCase()}`, defaultMessage: buildingState.currentBuilding.buildingType || '综合' })}</Tag></ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.address', defaultMessage: '地址' })} span={2}>{buildingState.currentBuilding.address || '-'}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.totalFloors', defaultMessage: '总楼层' })}>{buildingState.currentBuilding.totalFloors}层</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.yearBuilt', defaultMessage: '建成年份' })}>{buildingState.currentBuilding.yearBuilt || '-'}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.deliveryDate', defaultMessage: '交付/取得日期' })}>{buildingState.currentBuilding.deliveryDate ? dayjs(buildingState.currentBuilding.deliveryDate).format('YYYY-MM-DD') : '-'}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.totalArea', defaultMessage: '总面积' })}>{buildingState.currentBuilding.totalArea?.toLocaleString()} m²</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.rentedArea', defaultMessage: '已租面积' })}>{buildingState.currentBuilding.rentedArea?.toLocaleString()} m²</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.occupancyRate', defaultMessage: '出租率' })}><Tag color={(buildingState.currentBuilding.occupancyRate || 0) >= 80 ? 'green' : (buildingState.currentBuilding.occupancyRate || 0) >= 50 ? 'orange' : 'red'}>{buildingState.currentBuilding.occupancyRate || 0}%</Tag></ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.totalUnits', defaultMessage: '房源总数' })}>{buildingState.currentBuilding.totalUnits}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.availableUnits', defaultMessage: '可用房源' })}>{buildingState.currentBuilding.availableUnits}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.status', defaultMessage: '状态' })}><Tag color={buildingState.currentBuilding.status === 'Active' ? 'green' : 'orange'}>{intl.formatMessage({ id: `pages.park.asset.status.${buildingState.currentBuilding.status === 'Active' ? 'active' : 'maintenance'}`, defaultMessage: buildingState.currentBuilding.status === 'Active' ? '正常' : '维护中' })}</Tag></ProDescriptions.Item>
                    </ProDescriptions>
                ...
                {unitState.currentUnit && (<Space orientation="vertical" style={{ width: '100%' }} size={24}>
                    <div>
                        ...
                        <ProDescriptions bordered column={2} size="small">
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.unitNumber', defaultMessage: '房源编号' })}>{unitState.currentUnit.unitNumber}</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.buildingName', defaultMessage: '所属楼宇' })}>{unitState.currentUnit.buildingName}</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.floor', defaultMessage: '所在楼层' })}>{unitState.currentUnit.floor}F</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.area', defaultMessage: '房源面积' })}>{unitState.currentUnit.area} m²</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.unitType', defaultMessage: '房源类型' })}><Tag color={unitState.currentUnit.unitType === 'Office' ? 'blue' : unitState.currentUnit.unitType === 'Commercial' ? 'green' : 'purple'}>{intl.formatMessage({ id: `pages.park.asset.unitType.${unitState.currentUnit.unitType?.toLowerCase()}`, defaultMessage: unitState.currentUnit.unitType === 'Office' ? '办公' : unitState.currentUnit.unitType === 'Commercial' ? '商铺' : unitState.currentUnit.unitType || '其他' })}</Tag></ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.monthlyRent', defaultMessage: '月租金' })}>¥{unitState.currentUnit.monthlyRent?.toLocaleString()}</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.dailyRent', defaultMessage: '日租金' })}>{unitState.currentUnit.dailyRent ? `¥${unitState.currentUnit.dailyRent?.toLocaleString()}/m²` : '-'}</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.status', defaultMessage: '状态' })}><Tag color={unitState.currentUnit.status === 'Available' ? 'green' : 'blue'}>{intl.formatMessage({ id: `pages.park.asset.unit.status.${unitState.currentUnit.status?.toLowerCase()}`, defaultMessage: unitState.currentUnit.status === 'Available' ? '空置' : '已出租' })}</Tag></ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.currentTenant', defaultMessage: '当前租客' })}>{unitState.currentUnit.currentTenantName || '-'}</ProDescriptions.Item>
                            <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.asset.detail.field.leaseEndDate', defaultMessage: '租期到期' })} span={2}>{unitState.currentUnit.leaseEndDate ? dayjs(unitState.currentUnit.leaseEndDate).format('YYYY-MM-DD') : '-'}</ProDescriptions.Item>
                        </ProDescriptions>
