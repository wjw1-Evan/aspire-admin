...
import { ModalForm, ProFormText, ProFormSelect, ProFormDatePicker } from '@ant-design/pro-components/es/form';
        ...
        <PageContainer>
            <ProCard>
                <Tabs activeKey={state.activeTab} onChange={(key) => set({ activeTab: key })} items={[
                    { key: 'buildings', label: <Space><BankOutlined />{intl.formatMessage({ id: 'pages.park.asset.buildings' })}</Space>, children: <ProTable<Building> actionRef={buildingActionRef} headerTitle={<Space size={24}><Space><BankOutlined />资产管理</Space><Space size={12}><Tag color="blue">楼宇 {state.statistics?.totalBuildings || 0}</Tag><Tag color="green">房源 {state.statistics?.totalUnits || 0}</Tag><Tag color="purple">总面积 {state.statistics?.totalArea?.toLocaleString() || 0} m²</Tag><Tag color={(state.statistics?.occupancyRate ?? 0) >= 80 ? 'success' : (state.statistics?.occupancyRate ?? 0) >= 50 ? 'warning' : 'error'}>出租率 {state.statistics?.occupancyRate || 0}%</Tag></Space></Space>} request={async (params: any, sort: any, filter: any) => { const res = await api.buildings({ ...params, search: state.search, sort, filter }); return { data: res.data?.queryable || [], total: res.data?.rowCount || 0, success: res.success }; }} columns={buildingColumns} rowKey="id" search={false} toolBarRender={() => [<Input.Search key="search" placeholder={intl.formatMessage({ id: 'pages.common.search' })} allowClear value={state.search} onChange={(e) => set({ search: e.target.value })} onSearch={(v) => { set({ search: v }); buildingActionRef.current?.reload(); }} style={{ width: 260, marginRight: 8 }} prefix={<SearchOutlined />} />, <Button key="add" type="primary" icon={<PlusOutlined />} onClick={() => { setBuilding({ editingBuilding: null, modalVisible: true }); setForm({ attachments: [] }); }}>新增楼宇</Button>]} scroll={{ x: 1200 }} /> },
                    { key: 'units', label: <Space><HomeOutlined />{intl.formatMessage({ id: 'pages.park.asset.units' })}</Space>, children: <ProTable<PropertyUnit> actionRef={unitActionRef} headerTitle={<Space size={24}><Space><HomeOutlined />房源管理</Space><Space size={12}><Tag color="blue">房源 {state.statistics?.totalUnits || 0}</Tag><Tag color="green">可用 {state.statistics?.availableUnits || 0}</Tag><Tag color="purple">总面积 {state.statistics?.totalArea?.toLocaleString() || 0} m²</Tag><Tag color={(state.statistics?.occupancyRate ?? 0) >= 80 ? 'success' : (state.statistics?.occupancyRate ?? 0) >= 50 ? 'warning' : 'error'}>出租率 {state.statistics?.occupancyRate || 0}%</Tag></Space></Space>} request={async (params: any, sort: any, filter: any) => { const res = await api.units({ ...params, search: state.search, sort, filter }); return { data: res.data?.queryable || [], total: res.data?.rowCount || 0, success: res.success }; }} columns={unitColumns} rowKey="id" search={false} toolBarRender={() => [<Input.Search key="search" placeholder={intl.formatMessage({ id: 'pages.common.search' })} allowClear value={state.search} onChange={(e) => set({ search: e.target.value })} onSearch={(v) => { set({ search: v }); unitActionRef.current?.reload(); }} style={{ width: 260, marginRight: 8 }} prefix={<SearchOutlined />} />, <Button key="add" type="primary" icon={<PlusOutlined />} onClick={() => { setUnit({ editingUnit: null, modalVisible: true }); setForm({ attachments: [] }); }}>新增房源</Button>]} scroll={{ x: 1200 }} /> },
                ]} />
            </ProCard>
