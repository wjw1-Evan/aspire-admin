...
interface PropertyUnit {
  id: string; buildingName?: string; unitNumber: string; area?: number;
}
...
const ContractManagement: React.FC = () => {
  ...
  const columns: ProColumns<LeaseContract>[] = [
    ...
    { title: intl.formatMessage({ id: 'common.action', defaultMessage: '操作' }), valueType: 'option', width: 200, fixed: 'right', render: (_, record) => [
      ...
      <Button key="edit" type="link" icon={<EditOutlined />} onClick={() => {
        set({ isEdit: true, currentContract: record, contractModalVisible: true, fileList: (record.attachments || []).map(id => ({ uid: id, name: `附件-${id.substring(0, 8)}`, status: 'done', url: `/api/cloud-storage/files/${id}/download` })) });
      }}>{intl.formatMessage({ id: 'common.edit', defaultMessage: '编辑' })}</Button>,
      record.status === 'Active' && <Button key="renew" type="link" icon={<SyncOutlined />} onClick={() => {
        set({ isEdit: false, currentContract: record, contractModalVisible: true, fileList: [] });
      }}>{intl.formatMessage({ id: 'pages.park.contract.renew', defaultMessage: '续签' })}</Button>,
      <Popconfirm key="delete" title={intl.formatMessage({ id: 'common.confirmDelete', defaultMessage: '确认删除？' })} onConfirm={async () => { await api.delete(record.id); actionRef.current?.reload(); api.statistics().then(r => { if (r.success && r.data) set({ statistics: r.data }); }); }}>
        <Button type="link" danger icon={<DeleteOutlined />}>{intl.formatMessage({ id: 'common.delete', defaultMessage: '删除' })}</Button>
      </Popconfirm>,
    ]},
  ];
  ...
  const uploadProps: UploadProps = {
    ...
    onRemove: (file) => {
      ...
      if (index > -1) { const newList = state.fileList.slice(); newList.splice(index, 1); set({ fileList: newList }); }
    },
  };
  ...
  return (
    <PageContainer title="合同管理" extra={<Space><Button icon={<ReloadOutlined />} onClick={() => { actionRef.current?.reload(); api.statistics().then(r => { if (r.success && r.data) set({ statistics: r.data }); }); }}>刷新</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => set({ isEdit: false, currentContract: null, contractModalVisible: true, fileList: [] })}>新增合同</Button></Space>}>
      ...
      <ProTable actionRef={actionRef} request={async (params: any) => {
        ...
        toolBarRender={() => [
          <Input.Search key="search" placeholder="搜索..." style={{ width: 200 }} allowClear value={searchParams.search} onChange={(e) => setSearchParams({ search: e.target.value })} onSearch={(v) => { setSearchParams({ search: v }); actionRef.current?.reload(); }} />,
        ]}
      />
      ...
      <ModalForm key={state.isEdit && state.currentContract?.id ? state.currentContract.id : 'create'}
        ...
        onOpenChange={(open) => { if (!open) set({ contractModalVisible: false }); }}
        initialValues={state.currentContract ? {
          tenantId: state.currentContract.tenantId, contractNumber: state.currentContract.contractNumber,
          unitIds: state.currentContract.unitIds || [],
          startDate: state.currentContract.startDate ? [dayjs(state.currentContract.startDate), dayjs(state.currentContract.endDate)] : undefined,
          unitPrice: state.currentContract.unitPrice, monthlyRent: state.currentContract.monthlyRent,
          propertyFee: state.currentContract.propertyFee, deposit: state.currentContract.deposit,
          paymentCycle: state.currentContract.paymentCycle, totalAmount: state.currentContract.totalAmount,
          terms: state.currentContract.terms,
        } : undefined}
        onFinish={async (values) => {
          const data: ContractFormData = {
            tenantId: values.tenantId, contractNumber: values.contractNumber,
            unitIds: values.unitIds, startDate: values.startDate?.[0]?.toISOString(),
            endDate: values.startDate?.[1]?.toISOString(), unitPrice: values.unitPrice,
            monthlyRent: values.monthlyRent, propertyFee: values.propertyFee,
            deposit: values.deposit, paymentCycle: values.paymentCycle,
      ...
      >
        <Row gutter={16}>
          <Col span={12}><ProFormSelect name="tenantId" label="租户" placeholder="请选择租户" rules={[{ required: true }]} options={state.tenants.map(t => ({ label: t.tenantName, value: t.id }))} /></Col>
          <Col span={12}><ProFormText name="contractNumber" label="合同编号" placeholder="不填则自动生成" /></ProFormText></Col>
        </Row>
        ...
        <Row gutter={16}>
          <Col span={6}><ProFormText name="unitPrice" label="单价(元/㎡/天)" rules={[{ required: true }]} fieldProps={{ addonAfter: '元/㎡/天' }} /></Col>
          <Col span={6}><ProFormText name="monthlyRent" label="月租金" rules={[{ required: true }]} fieldProps={{ addonAfter: '元' }} /></Col>
          <Col span={6}><ProFormText name="propertyFee" label="物业费/月" rules={[{ required: true }]} fieldProps={{ addonAfter: '元' }} /></Col>
          <Col span={6}><ProFormText name="deposit" label="押金" rules={[{ required: true }]} fieldProps={{ addonAfter: '元' }} /></Col>
        </Row>
        <Row gutter={16}>
          <Col span={12}><ProFormSelect name="paymentCycle" label="付款周期" rules={[{ required: true }]} options={[{ label: '月付', value: 'Monthly' }, { label: '季付', value: 'Quarterly' }, { label: '年付', value: 'Yearly' }]} /></Col>
          <Col span={12}><ProFormText name="totalAmount" label="合同总额" rules={[{ required: true }]} fieldProps={{ addonAfter: '元' }} /></Col>
        </Row>
        ...
        {state.currentContract && (
          <div style={{ padding: '0 8px' }}>
            ...
            <div style={{ marginTop: 24 }}>
              ...
              <List size="small" dataSource={state.currentContract.paymentRecords || []} renderItem={(item: LeasePaymentRecord) => (
                <List.Item actions={[<Button type="link" danger size="small" icon={<DeleteOutlined />} onClick={async () => { await api.deletePayment(item.id); const updated = await api.get(state.currentContract!.id); if (updated.success && updated.data) set({ currentContract: updated.data }); }} />]}>
            ...
            {state.currentContract.attachments && state.currentContract.attachments.length > 0 && (
              <div style={{ marginTop: 24 }}>
                ...
                <List size="small" dataSource={state.currentContract.attachments} renderItem={(id) => (
      ...
      >
        <Row gutter={16}>
          <Col span={12}><ProFormText name="amount" label="付款金额" rules={[{ required: true }]} fieldProps={{ prefix: '¥' }} /></Col>
          <Col span={12}><ProFormSelect name="paymentType" label="款项类型" rules={[{ required: true }]} options={[{ label: '房租', value: 'Rent' }, { label: '物业费', value: 'PropertyFee' }, { label: '押金', value: 'Deposit' }, { label: '其他', value: 'Other' }]} /></Col>
        </Row>
        <Row gutter={16}>
          <Col span={12}><ProFormText name="paymentDate" label="付款日期" rules={[{ required: true }]} placeholder="选择日期" /></Col>
          <Col span={12}><ProFormSelect name="paymentMethod" label="付款方式" options={[{ label: '银行转账', value: 'BankTransfer' }, { label: '微信支付', value: 'WeChat' }, { label: '支付宝', value: 'Alipay' }, { label: '现金', value: 'Cash' }]} /></Col>
        </Row>
