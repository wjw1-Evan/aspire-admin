...
const contractStatusOptions = (intl: ReturnType<typeof useIntl>) => [
    { label: intl.formatMessage({ id: 'pages.park.contract.status.active', defaultMessage: '有效' }), value: 'Active', color: 'green' }, { label: intl.formatMessage({ id: 'pages.park.contract.status.pending', defaultMessage: '待生效' }), value: 'Pending', color: 'blue' },
    { label: intl.formatMessage({ id: 'pages.park.contract.status.expired', defaultMessage: '已到期' }), value: 'Expired', color: 'default' }, { label: intl.formatMessage({ id: 'pages.park.contract.status.renewed', defaultMessage: '已续签' }), value: 'Renewed', color: 'cyan' },
...
const ContractManagement: React.FC = () => {
    ...
    const columns: ProColumns<LeaseContract>[] = [
        ...
        { title: intl.formatMessage({ id: 'pages.park.contract.units', defaultMessage: '租用单元' }), dataIndex: 'unitNumbers', width: 200, render: (_, record) => <Space size={[0, 4]} wrap>{record.unitNumbers?.map(num => <Tag key={num} color="blue">{num}</Tag>)}{(!record.unitNumbers || record.unitNumbers.length === 0) && '-'}</Space> },
        ...
        { title: intl.formatMessage({ id: 'pages.park.contract.status', defaultMessage: '状态' }), dataIndex: 'status', sorter: true, width: 100, render: (status) => { const opt = contractStatusOptions(intl).find(o => o.value === status); return <Tag color={opt?.color || 'default'}>{opt?.label || status}</Tag>; } },
        {
    ...
    const uploadProps: UploadProps = {
        ...
        customRequest: async (options) => {
            ...
            try {
                ...
                } else { onError?.(new Error(res.message || intl.formatMessage({ id: 'pages.park.contract.message.uploadFailed', defaultMessage: '上传失败' }))); }
    ...
    return (
        <PageContainer>
            <ProTable actionRef={actionRef} headerTitle={
                    <Space size={24}>
                        <Space><FileTextOutlined />{intl.formatMessage({ id: 'pages.park.contract.contractManagement', defaultMessage: '合同管理' })}</Space>
                        <Space size={12}>
                            <Tag color="green">{intl.formatMessage({ id: 'pages.park.contract.statistics.active', defaultMessage: '生效' })} {state.statistics?.activeContracts || 0}</Tag>
                            <Tag color="blue">{intl.formatMessage({ id: 'pages.park.contract.statistics.totalAmount', defaultMessage: '总额' })} ¥{state.statistics?.totalContractAmount?.toLocaleString() || 0}</Tag>
                            <Tag color={state.statistics?.expiringContracts ? 'red' : 'default'}>{intl.formatMessage({ id: 'pages.park.contract.statistics.expiring', defaultMessage: '即将到期' })} {state.statistics?.expiringContracts || 0}</Tag>
                            <Tag color="purple">{intl.formatMessage({ id: 'pages.park.contract.statistics.expectedThisMonth', defaultMessage: '本月应收' })} ¥{state.statistics?.totalExpected?.toLocaleString() || 0}</Tag>
                        </Space>
                    </Space>
                ...
                toolBarRender={() => [
                    ...
                    />,
                    <Button key="add" type="primary" icon={<PlusOutlined />} onClick={() => set({ isEdit: false, currentContract: null, contractModalVisible: true, fileList: [] })}>{intl.formatMessage({ id: 'pages.park.contract.addContract', defaultMessage: '新增合同' })}</Button>,
                ]}
            />

            <ModalForm key={state.isEdit && state.currentContract?.id ? state.currentContract.id : 'create'}
                title={state.isEdit ? intl.formatMessage({ id: 'pages.park.contract.editContract', defaultMessage: '编辑合同' }) : state.currentContract ? intl.formatMessage({ id: 'pages.park.contract.renewContract', defaultMessage: '续签合同' }) : intl.formatMessage({ id: 'pages.park.contract.addContract', defaultMessage: '新增合同' })}
                ...
                onFinish={async (values) => {
                    ...
                    if (res.success) { message.success(state.isEdit ? intl.formatMessage({ id: 'pages.park.contract.message.updateSuccess', defaultMessage: '更新成功' }) : intl.formatMessage({ id: 'pages.park.contract.message.createSuccess', defaultMessage: '创建成功' })); set({ contractModalVisible: false, fileList: [] }); actionRef.current?.reload(); api.statistics().then(r => { if (r.success && r.data) set({ statistics: r.data }); }); }
                    return res.success;
                }} autoFocusFirstInput width={640}>
                <Row gutter={16}><Col span={12}><ProFormSelect name="tenantId" label={intl.formatMessage({ id: 'pages.park.contract.tenant', defaultMessage: '租户' })} placeholder={intl.formatMessage({ id: 'pages.park.contract.tenantPlaceholder', defaultMessage: '请选择租户' })} rules={[{ required: true }]} options={state.tenants.map(t => ({ label: t.tenantName, value: t.id }))} /></Col><Col span={12}><ProFormText name="contractNumber" label={intl.formatMessage({ id: 'pages.park.contract.number', defaultMessage: '合同编号' })} placeholder={intl.formatMessage({ id: 'pages.park.contract.numberPlaceholder', defaultMessage: '不填则自动生成' })} /></Col></Row>
                <ProFormSelect name="unitIds" label={intl.formatMessage({ id: 'pages.park.contract.units', defaultMessage: '租用单元' })} placeholder={intl.formatMessage({ id: 'pages.park.contract.unitsPlaceholder', defaultMessage: '请选择房源' })} rules={[{ required: true }]} mode="multiple" options={state.allUnits.map(u => ({ label: `${u.buildingName || ''} - ${u.unitNumber} (${u.area}m²)`, value: u.id }))} />
                <ProFormDateRangePicker name="startDate" label={intl.formatMessage({ id: 'pages.park.contract.period', defaultMessage: '租期' })} rules={[{ required: true }]} />
                <Row gutter={16}><Col span={6}><ProFormText name="unitPrice" label={intl.formatMessage({ id: 'pages.park.contract.unitPrice', defaultMessage: '单价(元/㎡/天)' })} rules={[{ required: true }]} fieldProps={{ addonAfter: intl.formatMessage({ id: 'pages.park.contract.unitPriceAddon', defaultMessage: '元/㎡/天' }) }} /></Col><Col span={6}><ProFormText name="monthlyRent" label={intl.formatMessage({ id: 'pages.park.contract.rent', defaultMessage: '月租金' })} rules={[{ required: true }]} fieldProps={{ addonAfter: intl.formatMessage({ id: 'pages.park.contract.currencyYuan', defaultMessage: '元' }) }} /></Col><Col span={6}><ProFormText name="propertyFee" label={intl.formatMessage({ id: 'pages.park.contract.propertyFee', defaultMessage: '物业费/月' })} rules={[{ required: true }]} fieldProps={{ addonAfter: intl.formatMessage({ id: 'pages.park.contract.currencyYuan', defaultMessage: '元' }) }} /></Col><Col span={6}><ProFormText name="deposit" label={intl.formatMessage({ id: 'pages.park.contract.deposit', defaultMessage: '押金' })} rules={[{ required: true }]} fieldProps={{ addonAfter: intl.formatMessage({ id: 'pages.park.contract.currencyYuan', defaultMessage: '元' }) }} /></Col></Row>
                <Row gutter={16}><Col span={12}><ProFormSelect name="paymentCycle" label={intl.formatMessage({ id: 'pages.park.contract.paymentCycle', defaultMessage: '付款周期' })} rules={[{ required: true }]} options={[{ label: intl.formatMessage({ id: 'pages.park.contract.paymentCycle.monthly', defaultMessage: '月付' }), value: 'Monthly' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentCycle.quarterly', defaultMessage: '季付' }), value: 'Quarterly' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentCycle.yearly', defaultMessage: '年付' }), value: 'Yearly' }]} /></Col><Col span={12}><ProFormText name="totalAmount" label={intl.formatMessage({ id: 'pages.park.contract.totalAmount', defaultMessage: '合同总额' })} rules={[{ required: true }]} fieldProps={{ addonAfter: intl.formatMessage({ id: 'pages.park.contract.currencyYuan', defaultMessage: '元' }) }} /></Col></Row>
                <div style={{ marginBottom: 16 }}><Typography.Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 8 }}>{intl.formatMessage({ id: 'pages.park.contract.attachments', defaultMessage: '合同附件' })}</Typography.Text><Upload {...uploadProps}><Button icon={<UploadOutlined />}>{intl.formatMessage({ id: 'pages.park.contract.uploadButton', defaultMessage: '点击上传' })}</Button></Upload></div>
                <ProFormText name="terms" label={intl.formatMessage({ id: 'pages.park.contract.terms', defaultMessage: '条款备注' })} placeholder={intl.formatMessage({ id: 'pages.park.contract.termsPlaceholder', defaultMessage: '备注信息' })} />
            </ModalForm>

            <Drawer title={state.currentContract?.contractNumber || intl.formatMessage({ id: 'pages.park.contract.contractDetail', defaultMessage: '合同详情' })} open={state.detailDrawerVisible} onClose={() => { set({ detailDrawerVisible: false, currentContract: null }); }} size="large" loading={state.detailLoading}>
                {state.currentContract && (<div style={{ padding: '0 8px' }}>
                    <ProDescriptions bordered column={2} size="small">
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.number', defaultMessage: '合同编号' })} span={2}>{state.currentContract.contractNumber}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.tenantName', defaultMessage: '租户名称' })} span={2}>{state.currentContract.tenantName}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.period', defaultMessage: '租期' })}>{dayjs(state.currentContract.startDate).format('YYYY-MM-DD')} ~ {dayjs(state.currentContract.endDate).format('YYYY-MM-DD')}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.rent', defaultMessage: '月租金' })}>¥{state.currentContract.monthlyRent?.toLocaleString()}</ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.status', defaultMessage: '状态' })}><Tag color={contractStatusOptions(intl).find(o => o.value === state.currentContract?.status)?.color}>{contractStatusOptions(intl).find(o => o.value === state.currentContract?.status)?.label || state.currentContract?.status}</Tag></ProDescriptions.Item>
                        <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.park.contract.paymentCycle', defaultMessage: '付款周期' })}>{state.currentContract.paymentCycle}</ProDescriptions.Item>
                    </ProDescriptions>
                    <div style={{ marginTop: 24 }}>
                        <Flex justify="space-between" align="center" style={{ marginBottom: 16 }}><Text strong>{intl.formatMessage({ id: 'pages.park.contract.paymentRecords', defaultMessage: '付款记录' })}</Text><Button type="primary" ghost size="small" icon={<PlusOutlined />} onClick={() => set({ paymentModalVisible: true })}>{intl.formatMessage({ id: 'pages.park.contract.addRecord', defaultMessage: '添加记录' })}</Button></Flex>
                        <List size="small" dataSource={state.currentContract.paymentRecords || []} renderItem={(item: LeasePaymentRecord) => (<List.Item actions={[<Button type="link" danger size="small" icon={<DeleteOutlined />} onClick={async () => { await api.deletePayment(item.id); const updated = await api.getDetail(state.currentContract!.id); if (updated.success && updated.data) set({ currentContract: updated.data }); }} />]}>
                            <List.Item.Meta title={<Space><Text strong>¥{item.amount.toLocaleString()}</Text><Tag color="blue">{item.paymentType || intl.formatMessage({ id: 'pages.park.contract.paymentType.rent', defaultMessage: '房租' })}</Tag></Space>} description={<Text type="secondary">{intl.formatMessage({ id: 'pages.park.contract.date', defaultMessage: '日期' })}: {dayjs(item.paymentDate).format('YYYY-MM-DD')} | {intl.formatMessage({ id: 'pages.park.contract.method', defaultMessage: '方式' })}: {item.paymentMethod}</Text>} />
                        </List.Item>)} />
                    </div>
                    {state.currentContract.attachments && state.currentContract.attachments.length > 0 && (<div style={{ marginTop: 24 }}>
                        <Text strong>{intl.formatMessage({ id: 'pages.park.contract.attachments', defaultMessage: '合同附件' })}</Text>
                        <List size="small" dataSource={state.currentContract.attachments} renderItem={(id) => (<List.Item actions={[<Button type="link" icon={<EyeOutlined />} href={`/apiservice/api/cloud-storage/files/${id}/preview`} target="_blank">{intl.formatMessage({ id: 'pages.park.contract.preview', defaultMessage: '预览' })}</Button>, <Button type="link" icon={<DownloadOutlined />} href={`/apiservice/api/cloud-storage/files/${id}/download`}>{intl.formatMessage({ id: 'pages.park.contract.download', defaultMessage: '下载' })}</Button>]}><List.Item.Meta avatar={<PaperClipOutlined />} title={`${intl.formatMessage({ id: 'pages.park.contract.file', defaultMessage: '文件' })}-${id.substring(0, 8)}`} /></List.Item>)} />
                    </div>)}
                </div>)}
            </Drawer>

            <ModalForm key={state.currentContract?.id || 'new'} title={intl.formatMessage({ id: 'pages.park.contract.addPaymentRecord', defaultMessage: '添加付款记录' })} open={state.paymentModalVisible} onOpenChange={(open) => { if (!open) set({ paymentModalVisible: false }); }}
                onFinish={async (values) => {
                    ...
                    if (res.success) { message.success(intl.formatMessage({ id: 'pages.park.contract.message.addSuccess', defaultMessage: '添加成功' })); set({ paymentModalVisible: false }); const updated = await api.getDetail(state.currentContract.id); if (updated.success && updated.data) set({ currentContract: updated.data }); }
                    return res.success;
                }} autoFocusFirstInput width={480}>
                <Row gutter={16}><Col span={12}><ProFormText name="amount" label={intl.formatMessage({ id: 'pages.park.contract.paymentAmount', defaultMessage: '付款金额' })} rules={[{ required: true }]} fieldProps={{ prefix: '¥' }} /></Col><Col span={12}><ProFormSelect name="paymentType" label={intl.formatMessage({ id: 'pages.park.contract.paymentType', defaultMessage: '款项类型' })} rules={[{ required: true }]} options={[{ label: intl.formatMessage({ id: 'pages.park.contract.paymentType.rent', defaultMessage: '房租' }), value: 'Rent' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentType.propertyFee', defaultMessage: '物业费' }), value: 'PropertyFee' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentType.deposit', defaultMessage: '押金' }), value: 'Deposit' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentType.other', defaultMessage: '其他' }), value: 'Other' }]} /></Col></Row>
                <Row gutter={16}><Col span={12}><ProFormDatePicker name="paymentDate" label={intl.formatMessage({ id: 'pages.park.contract.paymentDate', defaultMessage: '付款日期' })} rules={[{ required: true }]} placeholder={intl.formatMessage({ id: 'pages.park.contract.selectDate', defaultMessage: '选择日期' })} /></Col><Col span={12}><ProFormSelect name="paymentMethod" label={intl.formatMessage({ id: 'pages.park.contract.paymentMethod', defaultMessage: '付款方式' })} options={[{ label: intl.formatMessage({ id: 'pages.park.contract.paymentMethod.bankTransfer', defaultMessage: '银行转账' }), value: 'BankTransfer' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentMethod.wechat', defaultMessage: '微信支付' }), value: 'WeChat' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentMethod.alipay', defaultMessage: '支付宝' }), value: 'Alipay' }, { label: intl.formatMessage({ id: 'pages.park.contract.paymentMethod.cash', defaultMessage: '现金' }), value: 'Cash' }]} /></Col></Row>
                <ProFormDateRangePicker name="periodRange" label={intl.formatMessage({ id: 'pages.park.contract.periodRange', defaultMessage: '费用账期' })} />
                <ProFormText name="notes" label={intl.formatMessage({ id: 'pages.park.contract.notes', defaultMessage: '备注' })} placeholder={intl.formatMessage({ id: 'pages.park.contract.notesPlaceholder', defaultMessage: '备注信息' })} />
            </ModalForm>
        </PageContainer>
    );
};
