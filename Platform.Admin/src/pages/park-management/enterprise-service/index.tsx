        ...
        <PageContainer title={intl.formatMessage({ id: 'pages.park.service.title', defaultMessage: '企业服务' })} extra={
            ...
            <Card>
                <Tabs activeKey={state.activeTab} onChange={(key) => set({ activeTab: key })} items={[
                    { key: 'requests', label: <Space><FormOutlined />服务申请</Space>, children: <ProTable actionRef={actionRef} request={async (params: any) => { const { pageSize, current } = params; const sortParams = state.sorter?.sortBy && state.sorter?.sortOrder ? state.sorter : undefined; const res = await api.requests({ page: current, pageSize, search: state.searchText, ...sortParams }); loadData(); return { data: res.data?.queryable || [], total: res.data?.rowCount || 0, success: res.success }; }} columns={columns} rowKey="id" search={false} pagination={{ pageSizeOptions: ['10', '20', '50', '100'], defaultPageSize: 20 }} onChange={(_p, _f, s: any) => set({ sorter: s?.order ? { sortBy: s.field, sortOrder: s.order === 'ascend' ? 'asc' : 'desc' } : undefined })} toolBarRender={() => [<Input key="search" placeholder="搜索..." style={{ width: 200 }} allowClear />]} scroll={{ x: 1200 }} /> },
