...
const InvestmentManagement: React.FC = () => {
    ...
    const handleDeleteProject = async (id: string) => { const res = await api.deleteProject(id); if (res.success) { message.success(intl.formatMessage({ id: 'pages.park.investment.message.deleteSuccess', defaultMessage: '删除成功' })); projectsActionRef.current?.reload(); handleRefresh(); } else { message.error(getErrorMessage(res, 'pages.park.investment.message.deleteFailed')); } };
    const handleConvertToProject = async (leadId: string) => { const res = await api.convertLeadToProject(leadId); if (res.success) { message.success(intl.formatMessage({ id: 'pages.park.investment.message.convertSuccess', defaultMessage: '转换成功' })); set({ activeTab: 'projects' }); leadsActionRef.current?.reload(); projectsActionRef.current?.reload(); handleRefresh(); } else { message.error(getErrorMessage(res, 'pages.park.investment.message.convertFailed')); } };
