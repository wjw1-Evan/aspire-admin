...
import { ProTable, ProCard, PageContainer } from '@ant-design/pro-components';
...
const StatisticsPage: React.FC = () => {
    ...
    const handleGenerateAiReport = async () => {
        ...
        try {
            ...
            if (res.success && res.data) { try { const html = await marked.parse(res.data); set({ aiReportContent: html }); } catch { set({ aiReportContent: res.data }); } } else set({ aiReportContent: intl.formatMessage({ id: 'pages.park.statistics.generateFailed' }) });
         } catch { set({ aiReportContent: intl.formatMessage({ id: 'pages.park.statistics.generateFailed' }) }); }
            ...
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                ...
                <Col xs={24} sm={12} lg={6}>
                    <ProCard style={{ height: 140, padding: '12px 16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                            <div>
                                <div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.activeTenants' })}</div>
                                 <div style={{ fontSize: 22, fontWeight: 500, color: '#722ed1' }}>{tenant?.activeTenants || 0} {intl.formatMessage({ id: 'pages.park.statistics.tenantUnit' })}</div>
                             </div>
                         ...
                         <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.45)' }}>
                             {intl.formatMessage({ id: 'pages.park.statistics.expiringLabel' })} <span style={{ color: '#faad14' }}>{tenant?.expiringContracts || 0}</span> | {intl.formatMessage({ id: 'pages.park.statistics.workOrderLabel' })} <span style={{ color: (service?.pendingRequests || 0) > 0 ? '#faad14' : 'inherit' }}>{service?.pendingRequests || 0}</span> | {intl.formatMessage({ id: 'pages.park.statistics.ratingLabel' })} <span style={{ color: '#faad14' }}>{service?.averageRating || 5.0}★</span>
                         </div>
                    </ProCard>
                </Col>
            </Row>
            <Row gutter={[24, 24]}>
                <Col xs={24} lg={12}><ProCard title={<Space><FundProjectionScreenOutlined style={{ color: '#722ed1' }} />{intl.formatMessage({ id: 'pages.park.statistics.investmentOverview' })}</Space>} style={{ marginBottom: 24, borderRadius: 8 }}>
                     <Row gutter={[16, 16]}><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.totalLeadsLabel' })} value={investment?.totalLeads || 0} styles={{ content: { color: '#1890ff' } }} /></Col><Col span={12}><Statistic title={state.period === 'year' ? intl.formatMessage({ id: 'pages.park.statistics.newThisYear' }, { type: '' }) : intl.formatMessage({ id: 'pages.park.statistics.newThisMonth' }, { type: '' })} value={investment?.newLeadsThisMonth || 0} prefix={<ArrowUpOutlined />} styles={{ content: { color: '#52c41a' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.projectsInNegotiationLabel' })} value={investment?.projectsInNegotiation || 0} styles={{ content: { color: '#faad14' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.conversionRateLabel' })} value={Number(investment?.conversionRate || 0).toFixed(1)} suffix="%" styles={{ content: { color: (investment?.conversionRate || 0) >= 30 ? '#52c41a' : '#faad14' } }} /></Col></Row>
                 </ProCard></Col>
                 <Col xs={24} lg={12}><ProCard title={<Space><CustomerServiceOutlined style={{ color: '#13c2c2' }} />{intl.formatMessage({ id: 'pages.park.statistics.serviceOverview' })}</Space>} style={{ marginBottom: 24, borderRadius: 8 }}>
                     <Row gutter={[16, 16]}><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.totalRequestsLabel' })} value={service?.totalRequests || 0} styles={{ content: { color: '#1890ff' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.pendingLabel' })} value={service?.pendingRequests || 0} prefix={<ClockCircleOutlined />} styles={{ content: { color: '#faad14' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.completedLabel' })} value={service?.completedRequests || 0} prefix={<CheckCircleOutlined />} styles={{ content: { color: '#52c41a' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.avgRatingLabel' })} value={service?.averageRating || 0} precision={1} prefix={<StarOutlined style={{ color: '#faad14' }} />} suffix="/ 5" /></Col></Row>
                 </ProCard></Col>
            </Row>
        </div>);
    };
    ...
    const renderAssetAnalysis = () => {
        ...
        return (<div>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.assetScaleLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#1890ff' }}>{asset.totalBuildings} {intl.formatMessage({ id: 'pages.park.statistics.totalBuildings' })}</div></div><div style={{ background: 'linear-gradient(135deg, #e6f7ff, #f0f5ff)', padding: 8, borderRadius: 6 }}><BankOutlined style={{ fontSize: 16, color: '#1890ff' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalAreaLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#595959' }}>{asset.totalArea.toLocaleString()} {intl.formatMessage({ id: 'pages.park.statistics.areaSquareMeters' })}</div></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.rentableAreaLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#faad14' }}>{asset.totalRentableArea.toLocaleString()} {intl.formatMessage({ id: 'pages.park.statistics.areaSquareMeters' })}</div></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.occupancyRateLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: Number(areaPercent) >= 80 ? '#52c41a' : Number(areaPercent) >= 50 ? '#faad14' : '#f5222d' }}>{areaPercent}%</div></div><div style={{ background: 'linear-gradient(135deg, #f6ffed, #fffbe6)', padding: 8, borderRadius: 6 }}><span style={{ fontSize: 16, color: Number(areaPercent) >= 80 ? '#52c41a' : '#faad14' }}>%</span></div></div><Progress percent={Number(areaPercent)} showInfo={false} size="small" strokeColor={Number(areaPercent) >= 80 ? '#52c41a' : Number(areaPercent) >= 50 ? '#faad14' : '#f5222d'} /></ProCard></Col>
            </Row>
            <Row gutter={[24, 24]}>
                <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.areaDistribution' })}><div style={{ textAlign: 'center', padding: 24 }}><Progress type="dashboard" percent={Number(areaPercent)} format={(percent) => (<div><div style={{ fontSize: 24, fontWeight: 'bold' }}>{percent}%</div><div style={{ fontSize: 12, color: '#999' }}>{intl.formatMessage({ id: 'pages.park.statistics.rented' })}</div></div>)} size={180} strokeColor={{ '0%': '#108ee9', '100%': '#87d068' }} /><Row gutter={16} style={{ marginTop: 24 }}><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.rentedAreaLabel2' })} value={asset.rentedArea} suffix={intl.formatMessage({ id: 'pages.park.statistics.areaSquareMeters' })} styles={{ content: { color: '#52c41a' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.availableUnitsLabel' })} value={asset.totalRentableArea - asset.rentedArea} suffix={intl.formatMessage({ id: 'pages.park.statistics.areaSquareMeters' })} styles={{ content: { color: '#faad14' } }} /></Col></Row></div></ProCard></Col>
                 <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.unitDistribution' })}><div style={{ textAlign: 'center', padding: 24 }}><Progress type="dashboard" percent={asset.totalUnits > 0 ? Number(((asset.rentedUnits / asset.totalUnits) * 100).toFixed(1)) : 0} format={(percent) => (<div><div style={{ fontSize: 24, fontWeight: 'bold' }}>{percent}%</div><div style={{ fontSize: 12, color: '#999' }}>{intl.formatMessage({ id: 'pages.park.statistics.rentedUnitsLabel' })}</div></div>)} size={180} strokeColor={{ '0%': '#722ed1', '100%': '#eb2f96' }} /><Row gutter={16} style={{ marginTop: 24 }}><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.rentedUnitsLabel' })} value={asset.rentedUnits} suffix={intl.formatMessage({ id: 'pages.park.statistics.unit' })} styles={{ content: { color: '#52c41a' } }} /></Col><Col span={12}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.availableUnitsLabel' })} value={asset.availableUnits} suffix={intl.formatMessage({ id: 'pages.park.statistics.unit' })} styles={{ content: { color: '#faad14' } }} /></Col></Row></div></ProCard></Col>
            </Row>
        </div>);
    };
    ...
    const renderInvestmentAnalysis = () => {
        ...
        return (<div>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalLeadsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#1890ff' }}>{investment.totalLeads}</div></div><div style={{ background: 'linear-gradient(135deg, #e6f7ff, #f0f5ff)', padding: 8, borderRadius: 6 }}><UserSwitchOutlined style={{ fontSize: 16, color: '#1890ff' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{state.period === 'year' ? intl.formatMessage({ id: 'pages.park.statistics.newThisYear' }, { type: '' }) : intl.formatMessage({ id: 'pages.park.statistics.newThisMonth' }, { type: '' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#52c41a' }}>{investment.newLeadsThisMonth}</div></div><div style={{ background: 'linear-gradient(135deg, #f6ffed, #f9f6f2)', padding: 8, borderRadius: 6 }}><ArrowUpOutlined style={{ fontSize: 16, color: '#52c41a' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalProjectsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#722ed1' }}>{investment.totalProjects}</div></div><div style={{ background: 'linear-gradient(135deg, #f9f0ff, #efdbff)', padding: 8, borderRadius: 6 }}><FundProjectionScreenOutlined style={{ fontSize: 16, color: '#722ed1' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={6}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.conversionRateLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: investment.conversionRate >= 30 ? '#52c41a' : '#faad14' }}>{Number(investment.conversionRate).toFixed(1)}%</div></div><div style={{ background: 'linear-gradient(135deg, #fffbe6, #fff7e6)', padding: 8, borderRadius: 6 }}><span style={{ fontSize: 16, color: investment.conversionRate >= 30 ? '#52c41a' : '#faad14' }}>%</span></div></div></ProCard></Col>
            </Row>
            <Row gutter={[24, 24]}>
                <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.leadsByStatusLabel' })}>{leadsByStatusData.length > 0 ? (<ProTable dataSource={leadsByStatusData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.status' }), dataIndex: 'status', render: (status) => <Tag>{status}</Tag> }, { title: intl.formatMessage({ id: 'pages.park.statistics.count' }), dataIndex: 'count', render: (count) => <Text strong>{count}</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record) => { const percent = investment.totalLeads > 0 ? ((record.count / investment.totalLeads) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" />; } }]} />) : <Empty />}</ProCard></Col>
                 <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.projectsByStageLabel' })}>{projectsByStageData.length > 0 ? (<ProTable dataSource={projectsByStageData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.stage' }), dataIndex: 'stage', render: (stage) => <Tag color="purple">{stage}</Tag> }, { title: intl.formatMessage({ id: 'pages.park.statistics.count' }), dataIndex: 'count', render: (count) => <Text strong>{count}</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record) => { const percent = investment.totalProjects > 0 ? ((record.count / investment.totalProjects) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" strokeColor="#722ed1" />; } }]} />) : <Empty />}</ProCard></Col>
            </Row>
        </div>);
    };
    ...
    const renderTenantAnalysis = () => {
        ...
        return (<div>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalTenantsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#1890ff' }}>{tenant.totalTenants}</div></div><div style={{ background: 'linear-gradient(135deg, #e6f7ff, #f0f5ff)', padding: 8, borderRadius: 6 }}><TeamOutlined style={{ fontSize: 16, color: '#1890ff' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.activeTenantsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#52c41a' }}>{tenant.activeTenants}</div></div><div style={{ background: 'linear-gradient(135deg, #f6ffed, #f9f6f2)', padding: 8, borderRadius: 6 }}><CheckCircleOutlined style={{ fontSize: 16, color: '#52c41a' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.activeContractsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#722ed1' }}>{tenant.activeContracts}</div></div><div style={{ background: 'linear-gradient(135deg, #f9f0ff, #efdbff)', padding: 8, borderRadius: 6 }}><FileTextOutlined style={{ fontSize: 16, color: '#722ed1' }} /></div></div></ProCard></Col>
            </Row>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalReceived' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#cf1322' }}>¥{(tenant.totalReceived || 0).toLocaleString()}</div></div><div style={{ background: 'linear-gradient(135deg, #fff1f0, #fff7e6)', padding: 8, borderRadius: 6 }}><span style={{ fontSize: 16, color: '#cf1322' }}>¥</span></div></div><div style={{ fontSize: 11, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.receivableLabel' })} ¥{(tenant.totalExpected || 0).toLocaleString()}</div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.yearlyEstimateLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#faad14' }}>¥{(tenant.yearlyEstimate || 0).toLocaleString()}</div></div><div style={{ background: 'linear-gradient(135deg, #fffbe6, #fff7e6)', padding: 8, borderRadius: 6 }}><span style={{ fontSize: 16, color: '#faad14' }}>¥</span></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.contractAmountLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#13c2c2' }}>¥{(tenant.totalContractAmount || 0).toLocaleString()}</div></div><div style={{ background: 'linear-gradient(135deg, #e6fffa, #f0f5ff)', padding: 8, borderRadius: 6 }}><span style={{ fontSize: 16, color: '#13c2c2' }}>¥</span></div></div></ProCard></Col>
            </Row>
            <Row gutter={[24, 24]}>
                <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.industryDistribution' })}>{industryData.length > 0 ? (<ProTable dataSource={industryData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.industry' }), dataIndex: 'industry', render: (industry) => <Tag color="blue">{industry}</Tag> }, { title: intl.formatMessage({ id: 'pages.park.statistics.tenantCount' }), dataIndex: 'count', render: (count) => <Text strong>{count}</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record) => { const percent = tenant.totalTenants > 0 ? ((record.count / tenant.totalTenants) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" />; } }]} />) : <Empty />}</ProCard></Col>
                 <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.paymentComposition' })}>{paymentTypeData.length > 0 ? (<ProTable dataSource={paymentTypeData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.paymentType' }), dataIndex: 'type', render: (t) => <Tag color="cyan">{t}</Tag> }, { title: intl.formatMessage({ id: 'pages.park.statistics.receivedAmount' }), dataIndex: 'amount', render: (a) => a !== null && a !== undefined ? <Text strong>¥{a.toLocaleString()}</Text> : <Text strong>-</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record) => { const percent = tenant.totalReceived > 0 ? ((record.amount / tenant.totalReceived) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" strokeColor="#13c2c2" />; } }]} />) : (<div style={{ textAlign: 'center', padding: '40px 0' }}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.totalReceivedLabel' })} value={tenant.totalReceived} prefix="¥" precision={2} /><Empty description={intl.formatMessage({ id: 'pages.park.statistics.noDetailData' })} /></div>)}</ProCard></Col>
            </Row>
            <Row gutter={[24, 24]} style={{ marginTop: 24 }}><Col span={24}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.collectionStatus' })}><Row gutter={16}><Col span={6}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.totalExpectedAmountLabel' })} value={tenant.totalExpected} prefix="¥" precision={2} /></Col><Col span={6}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.totalReceivedLabel' })} value={tenant.totalReceived} prefix="¥" precision={2} styles={{ content: { color: '#52c41a' } }} /></Col><Col span={6}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.expiringContractsLabel' })} value={tenant.expiringContracts} styles={{ content: { color: tenant.expiringContracts > 0 ? '#faad14' : '#52c41a' } }} prefix={tenant.expiringContracts > 0 ? <WarningOutlined /> : <CheckCircleOutlined />} /></Col><Col span={6}><Statistic title={intl.formatMessage({ id: 'pages.park.statistics.collectionRateLabel' })} value={tenant.collectionRate} suffix="%" precision={1} styles={{ content: { color: tenant.collectionRate >= 90 ? '#52c41a' : '#faad14' } }} /><Progress percent={tenant.collectionRate} showInfo={false} status={tenant.collectionRate >= 90 ? 'success' : 'active'} strokeColor={tenant.collectionRate >= 90 ? '#52c41a' : '#faad14'} /></Col></Row></ProCard></Col></Row>
        </div>);
    };
    ...
    const renderServiceAnalysis = () => {
        ...
        return (<div>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={12}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.totalRequestsLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#1890ff' }}>{service.totalRequests}</div></div><div style={{ background: 'linear-gradient(135deg, #e6f7ff, #f0f5ff)', padding: 8, borderRadius: 6 }}><CustomerServiceOutlined style={{ fontSize: 16, color: '#1890ff' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={12}><ProCard style={{ height: 140, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.avgRatingLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#faad14' }}>{service.averageRating} / 5</div></div><div style={{ background: 'linear-gradient(135deg, #fffbe6, #fff7e6)', padding: 8, borderRadius: 6 }}><StarOutlined style={{ fontSize: 16, color: '#faad14' }} /></div></div></ProCard></Col>
            </Row>
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 100, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.pendingLabel' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: service.pendingRequests > 10 ? '#f5222d' : '#faad14' }}>{service.pendingRequests}</div></div><div style={{ background: 'linear-gradient(135deg, #fffbe6, #fff7e6)', padding: 8, borderRadius: 6 }}><ClockCircleOutlined style={{ fontSize: 16, color: service.pendingRequests > 10 ? '#f5222d' : '#faad14' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 100, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.processingRequests' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#1890ff' }}>{service.processingRequests}</div></div><div style={{ background: 'linear-gradient(135deg, #e6f7ff, #f0f5ff)', padding: 8, borderRadius: 6 }}><SyncOutlined spin style={{ fontSize: 16, color: '#1890ff' }} /></div></div></ProCard></Col>
                 <Col xs={24} sm={12} lg={8}><ProCard style={{ height: 100, padding: '12px 16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}><div><div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{intl.formatMessage({ id: 'pages.park.statistics.completedRequests' })}</div><div style={{ fontSize: 22, fontWeight: 500, color: '#52c41a' }}>{service.completedRequests}</div></div><div style={{ background: 'linear-gradient(135deg, #f6ffed, #f9f6f2)', padding: 8, borderRadius: 6 }}><CheckCircleOutlined style={{ fontSize: 16, color: '#52c41a' }} /></div></div></ProCard></Col>
            </Row>
            <Row gutter={[24, 24]}>
                <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.requestStatusDistribution' })}>{statusData.length > 0 ? (<ProTable dataSource={statusData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.status' }), dataIndex: 'status', render: (status: any) => { const colorMap: Record<string, string> = { Pending: 'orange', Processing: 'blue', Completed: 'green', Cancelled: 'default' }; return <Tag color={colorMap[status] || 'default'}>{status}</Tag>; } }, { title: intl.formatMessage({ id: 'pages.park.statistics.count' }), dataIndex: 'count', render: (count) => <Text strong>{count}</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record: any) => { const percent = service.totalRequests > 0 ? ((record.count / service.totalRequests) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" />; } }]} />) : <Empty />}</ProCard></Col>
                 <Col xs={24} lg={12}><ProCard title={intl.formatMessage({ id: 'pages.park.statistics.requestCategoryStatistics' })}>{categoryData.length > 0 ? (<ProTable dataSource={categoryData} pagination={false} size="small" search={false} options={false} toolBarRender={false} columns={[{ title: intl.formatMessage({ id: 'pages.park.statistics.category' }), dataIndex: 'category', render: (category) => <Tag color="cyan">{category}</Tag> }, { title: intl.formatMessage({ id: 'pages.park.statistics.count' }), dataIndex: 'count', render: (count) => <Text strong>{count}</Text> }, { title: intl.formatMessage({ id: 'pages.park.statistics.percentage' }), render: (_: any, record: any) => { const percent = service.totalRequests > 0 ? ((record.count / service.totalRequests) * 100).toFixed(1) : 0; return <Progress percent={Number(percent)} size="small" strokeColor="#13c2c2" />; } }]} />) : <Empty />}</ProCard></Col>
            </Row>
        </div>);
    };
