...
const TenantManagement: React.FC = () => {
  ...
  const handleViewTenant = async (tenant: ParkTenant) => {
    set({ viewingTenant: tenant, detailVisible: true });
    setDetail({ loading: true, contracts: [], serviceRequests: [], payments: [] });
    try {
      const [serviceRes, contractRes] = await Promise.all([
        api.getServiceRequests({ page: 1, pageSize: 100, tenantId: tenant.id }),
        api.getContracts({ page: 1, pageSize: 100, tenantId: tenant.id }),
      ]);
      ...
      if (contractRes.success && contractRes.data) {
        ...
        for (const contract of contractRes.data.queryable) {
          ...
          if (paymentRes.success && paymentRes.data) allPayments.push(...paymentRes.data.map(p => ({ ...p, contractNumber: contract.contractNumber })));
        }
  ...
  const handleDeleteTenant = async (id: string) => {
    ...
    if (res.success) { message.success('删除成功'); actionRef.current?.reload(); loadStatistics(); }
    ...
    <PageContainer title={<Space><UserOutlined />租户管理</Space>}
      extra={
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => set({ editingTenant: null, formVisible: true })}>新增租户</Button>
        </Space>
      }>
      ...
      <ModalForm key={state.editingTenant?.id || 'create'} title={state.editingTenant ? '编辑租户' : '新增租户'} open={state.formVisible}
        ...
        onFinish={async (values) => {
          const data = { ...values, entryDate: values.entryDate?.toISOString() };
          const res = state.editingTenant ? await api.update(state.editingTenant.id, data) : await api.create(data);
          if (res.success) { message.success(state.editingTenant ? '更新成功' : '创建成功'); set({ formVisible: false, editingTenant: null }); actionRef.current?.reload(); loadStatistics(); }
          return res.success;
        }} autoFocusFirstInput width={640}>
        <Row gutter={16}>
          <Col span={12}><ProFormText name="tenantName" label="租户名称" placeholder="请输入租户名称" rules={[{ required: true, message: '请输入租户名称' }]} /></Col>
          <Col span={12}><ProFormText name="industry" label="行业" placeholder="请输入行业" /></Col>
        </Row>
        <Row gutter={16}>
          <Col span={8}><ProFormText name="contactPerson" label="联系人" placeholder="联系人" /></Col>
          <Col span={8}><ProFormText name="phone" label="电话" placeholder="电话" /></Col>
          <Col span={8}><ProFormText name="email" label="邮箱" placeholder="邮箱" /></Col>
        </Row>
        <Row gutter={16}>
          <Col span={12}><ProFormText name="businessLicense" label="营业执照号" placeholder="营业执照号" /></Col>
          <Col span={12}><ProFormDatePicker name="entryDate" label="入驻日期" style={{ width: '100%' }} /></Col>
        </Row>
      ...
      <Drawer title={state.viewingTenant?.tenantName || '租户详情'} open={state.detailVisible} onClose={() => set({ detailVisible: false, viewingTenant: null })} size={720} loading={detailData.loading}>
        {state.viewingTenant && <div>
          ...
          {detailData.contracts.length > 0 ? <Table size="small" dataSource={detailData.contracts} rowKey="id" pagination={false} style={{ marginBottom: 24 }} columns={[
          ...
          {detailData.serviceRequests.length > 0 ? <Table size="small" dataSource={detailData.serviceRequests} rowKey="id" pagination={false} style={{ marginBottom: 24 }} columns={[
            { title: '标题', dataIndex: 'title', ellipsis: true },
            { title: '类别', dataIndex: 'categoryName', width: 80 },
            { title: '状态', dataIndex: 'status', width: 80, render: (s: string) => { const statusMap: Record<string, { label: string; color: string }> = { 'Pending': { label: '待处理', color: 'orange' }, 'Processing': { label: '处理中', color: 'processing' }, 'Completed': { label: '已完成', color: 'green' }, 'Cancelled': { label: '已取消', color: 'default' } }; return <Tag color={statusMap[s]?.color}>{statusMap[s]?.label || s}</Tag>; } },
          ...
          {detailData.payments.length > 0 ? <Table size="small" dataSource={detailData.payments} rowKey="id" pagination={false} columns={[
            { title: '合同', dataIndex: 'contractNumber', width: 100 },
            { title: '类型', dataIndex: 'paymentType', width: 80, render: (t: string) => { const typeMap: Record<string, { label: string; color: string }> = { 'Rent': { label: '房租', color: 'blue' }, 'PropertyFee': { label: '物业费', color: 'orange' }, 'Deposit': { label: '押金', color: 'purple' }, 'Other': { label: '其他', color: 'default' } }; return <Tag color={typeMap[t]?.color || 'default'}>{typeMap[t]?.label || t || '房租'}</Tag>; } },
            { title: '金额', dataIndex: 'amount', width: 100, render: (v: number) => <Typography.Text type="success">¥{v?.toLocaleString()}</Typography.Text> },
