...
const api = {
  list: (params: PageParams & { status?: string }) =>
    request<ApiResponse<PagedResult<VisitTask>>>('/api/park-management/visit/tasks', { params }),
  createAssessment: (data: AssessmentFormData) =>
    request<ApiResponse<VisitAssessment>>('/api/park-management/visit/assessment', { method: 'POST', data }),
};
...
const VisitAssessmentList: React.FC = () => {
  ...
  const columns: ProColumns<VisitTask>[] = [
    ...
    { title: intl.formatMessage({ id: 'pages.park.visit.score', defaultMessage: '满意度' }), dataIndex: 'assessmentScore', width: 150, render: (dom: number | undefined) => dom !== undefined && dom !== null ? <Rate disabled value={dom} character={<StarFilled />} style={{ fontSize: 14 }} /> : <Tag color="warning">待评价</Tag> },
  ...
  const handleAssessmentSubmit = async (values: any) => {
    ...
    if (res.success) {
      ...
      set({ assessmentVisible: false, selectedTask: null });
      form.resetFields();
  ...
  return (
    <PageContainer title={intl.formatMessage({ id: 'pages.park.visit.assessment', defaultMessage: '走访评价管理' })}>
      ...
      <Drawer title={intl.formatMessage({ id: 'pages.park.visit.assessmentDetail', defaultMessage: '走访评价详情' })} placement="right" open={state.detailVisible} onClose={() => set({ detailVisible: false, selectedAssessment: null })} size={500}>
        {state.selectedAssessment && <div className={styles.detailContainer}>
          ...
          <div style={{ padding: '0 4px' }}><Text strong>{intl.formatMessage({ id: 'pages.park.visit.taskDescription', defaultMessage: '任务内容描述' })}：</Text><div style={{ marginTop: 8, padding: 12, background: '#f5f5f5', borderRadius: 8 }}>{state.selectedAssessment.taskDescription || '-'}</div></div>
          ...
          <div style={{ padding: '0 4px', marginBottom: 24 }}><Text strong>{intl.formatMessage({ id: 'pages.park.visit.comments', defaultMessage: '详细评语' })}：</Text><div style={{ marginTop: 8, padding: 12, border: '1px solid #f0f0f0', borderRadius: 8, minHeight: 100 }}>{state.selectedAssessment.comments || intl.formatMessage({ id: 'pages.park.common.noData', defaultMessage: '暂无评价内容' })}</div></div>
          <Divider />
