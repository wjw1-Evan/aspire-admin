...
const VisitStatisticsPage: React.FC = () => {
    ...
    const managerColumns: ProColumns<{ name: string; count: number }>[] = [
        { title: intl.formatMessage({ id: 'pages.table.name', defaultMessage: '姓名' }), dataIndex: 'name', render: (text: string, _, index: number) => (<Space>{index < 3 ? <Tag color={index === 0 ? 'gold' : index === 1 ? 'silver' : 'orange'}>{index + 1}</Tag> : <Tag>{index + 1}</Tag>}{text}</Space>) },
        ...
        <PageContainer title={intl.formatMessage({ id: 'pages.park.visit.statistics', defaultMessage: '走访统计报表' })} extra={[
            ...
            <Spin spinning={state.loading}>
                {state.statistics && (<Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                    <Col xs={24} sm={12} lg={6}><Card style={{ height: '100%', borderRadius: 8, transition: 'all 0.3s' }}><Statistic title={intl.formatMessage({ id: 'pages.park.visit.statistics.pendingTasks', defaultMessage: '待处理任务' })} value={state.statistics.pendingTasks} prefix={<ClockCircleOutlined />} styles={{ content: { color: '#faad14' } }} /><div style={{ color: 'rgba(0, 0, 0, 0.45)', fontSize: 12, marginTop: 4 }}>{intl.formatMessage({ id: 'pages.park.visit.statistics.overview', defaultMessage: '当前待执行的走访任务' })}</div></Card></Col>
                    <Col xs={24} sm={12} lg={6}><Card style={{ height: '100%', borderRadius: 8, transition: 'all 0.3s' }}><Statistic title={intl.formatMessage({ id: 'pages.park.visit.statistics.completedTasksThisMonth', defaultMessage: '本月完成任务' })} value={state.statistics.completedTasksThisMonth} prefix={<CheckCircleOutlined />} styles={{ content: { color: '#52c41a' } }} /><div style={{ color: 'rgba(0, 0, 0, 0.45)', fontSize: 12, marginTop: 4 }}>{intl.formatMessage({ id: 'pages.park.visit.statistics.completionRate', defaultMessage: '任务完成率' })}: {state.statistics.completionRate}%<Progress percent={state.statistics.completionRate} size="small" showInfo={false} strokeColor="#52c41a" /></div></Card></Col>
                    <Col xs={24} sm={12} lg={6}><Card style={{ height: '100%', borderRadius: 8, transition: 'all 0.3s' }}><Statistic title={intl.formatMessage({ id: 'pages.park.visit.statistics.activeManagers', defaultMessage: '活跃企管员' })} value={state.statistics.activeManagers} prefix={<TeamOutlined />} styles={{ content: { color: '#1890ff' } }} /><div style={{ color: 'rgba(0, 0, 0, 0.45)', fontSize: 12, marginTop: 4 }}>{intl.formatMessage({ id: 'pages.park.visit.statistics.managerRanking', defaultMessage: '参与走访的企管员人数' })}</div></Card></Col>
                    <Col xs={24} sm={12} lg={6}><Card style={{ height: '100%', borderRadius: 8, transition: 'all 0.3s' }}><Statistic title={intl.formatMessage({ id: 'pages.park.visit.statistics.averageScore', defaultMessage: '满意度评分' })} value={state.statistics.averageScore} prefix={<StarOutlined style={{ color: '#faad14' }} />} suffix="/ 5" /><div style={{ color: 'rgba(0, 0, 0, 0.45)', fontSize: 12, marginTop: 4 }}>{intl.formatMessage({ id: 'pages.park.visit.statistics.totalAssessments', defaultMessage: '评价总数' })}: {state.statistics.totalAssessments}</div></Card></Col>
                </Row>)}

                {state.statistics && (<Row gutter={[24, 24]}>
                    <Col xs={24} lg={12}><Card title={<Space><BarChartOutlined style={{ color: '#1890ff' }} />{intl.formatMessage({ id: 'pages.park.visit.statistics.tasksByType', defaultMessage: '任务类型分布' })}</Space>} style={{ marginBottom: 24, borderRadius: 8 }}><ProTable dataSource={typeData} pagination={false} size="small" rowKey="type" columns={typeColumns} search={false} /></Card></Col>
                    <Col xs={24} lg={12}><Card title={<Space><UserOutlined style={{ color: '#722ed1' }} />{intl.formatMessage({ id: 'pages.park.visit.statistics.managerRanking', defaultMessage: '企管员走访排行' })}</Space>} style={{ marginBottom: 24, borderRadius: 8 }}><ProTable dataSource={managerData} pagination={false} size="small" rowKey="name" columns={managerColumns} search={false} /></Card></Col>
