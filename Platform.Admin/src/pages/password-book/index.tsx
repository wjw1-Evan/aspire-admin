...
const api = {
  ...
  create: (data: Partial<Entry>) =>
    request<ApiResponse<Entry>>('/apiservice/api/password-book', { method: 'POST', data }),
  update: (id: string, data: Partial<Entry>) =>
    request<ApiResponse<Entry>>(`/apiservice/api/password-book/${id}`, { method: 'PUT', data }),
  statistics: () => request<ApiResponse<Stats>>('/apiservice/api/password-book/statistics'),
  export: (format: string) =>
    request<any>('/apiservice/api/password-book/export', {
      method: 'POST',
      data: { format },
      responseType: 'blob',
      getResponse: true,
  ...
  const columns: ProColumns<Entry>[] = [
{
    ...
    {
      title: intl.formatMessage({ id: 'pages.passwordBook.table.url' }),
      dataIndex: 'url',
      key: 'url',
      sorter: true,
      render: (dom) =>
        dom ? (
          <a href={dom as string} target="_blank" rel="noopener">
            {dom}
          </a>
        ) : (
          '-'
    ...
    {
      title: intl.formatMessage({ id: 'pages.passwordBook.table.category' }),
      dataIndex: 'category',
      key: 'category',
      sorter: true,
    ...
    {
      title: intl.formatMessage({ id: 'pages.passwordBook.table.tags' }),
      dataIndex: 'tags',
      render: (dom) =>
        dom && typeof dom === 'object' && 'length' in dom && (dom as string[]).length > 0 ? (
          <Space size={[0, 4]} wrap>
            {(dom as string[]).map((t) => (
              <Tag key={t}>{t}</Tag>
            ))}
    ...
    {
      title: intl.formatMessage({ id: 'pages.passwordBook.table.lastUsedAt' }),
    ...
    },
    {
      ...
      render: (_, r) => (
        <Space size={4}>
          ...
          <Popconfirm
            title={intl.formatMessage({ id: 'pages.passwordBook.message.confirmDelete' }, { name: r.platform })}
            onConfirm={async () => {
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        headerTitle={
          <Space size={24}>
            <Space>
              <LockOutlined />
              {intl.formatMessage({ id: 'pages.passwordBook.title' })}
            </Space>
            <Space size={12}>
              <Tag color="blue">
                {intl.formatMessage({ id: 'pages.passwordBook.statistics.totalEntries' })}{' '}
                {state.statistics?.totalEntries || 0}
              </Tag>
              <Tag color="green">
                {intl.formatMessage({ id: 'pages.passwordBook.statistics.categoryCount' })}{' '}
                {state.statistics?.categoryCount || 0}
              </Tag>
              <Tag color="orange">
                {intl.formatMessage({ id: 'pages.passwordBook.statistics.tagCount' })} {state.statistics?.tagCount || 0}
              </Tag>
              <Tag color="purple">
                {intl.formatMessage({ id: 'pages.passwordBook.statistics.recentUsedCount' })}{' '}
                {state.statistics?.recentUsedCount || 0}
              </Tag>
            </Space>
          </Space>
        }
        ...
        toolBarRender={() => [
          ...
          />,
          <Button key="export" icon={<ExportOutlined />} onClick={() => set({ exportVisible: true })}>
            {intl.formatMessage({ id: 'pages.passwordBook.action.export' })}
          </Button>,
          <Button
            key="create"
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => set({ editingEntry: null, formVisible: true })}
          >
      ...
      <ModalForm
        ...
        title={
          state.editingEntry
            ? intl.formatMessage({ id: 'pages.passwordBook.form.edit' })
            : intl.formatMessage({ id: 'pages.passwordBook.form.create' })
        }
        ...
        initialValues={
          state.editingEntry
            ? {
                ...
                password: state.editingEntry.password,
                url: state.editingEntry.url,
                category: state.editingEntry.category ? [state.editingEntry.category] : [],
                tags: state.editingEntry.tags || [],
      ...
      >
        <ProFormText
          name="platform"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.platform' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.platformPlaceholder' })}
          rules={[{ required: true, message: intl.formatMessage({ id: 'pages.passwordBook.form.platformRequired' }) }]}
        />
        <ProFormText
          name="account"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.account' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.accountPlaceholder' })}
          rules={[{ required: true, message: intl.formatMessage({ id: 'pages.passwordBook.form.accountRequired' }) }]}
        />
        <Form.Item
          name="password"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.password' })}
          rules={[{ required: true, message: intl.formatMessage({ id: 'pages.passwordBook.form.passwordRequired' }) }]}
        >
          <Input.Password placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.passwordPlaceholder' })} />
        </Form.Item>
        <ProFormText
          name="url"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.url' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.urlPlaceholder' })}
        />
        <ProFormSelect
          name="category"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.category' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.categoryPlaceholder' })}
          showSearch
          allowClear
          mode="tags"
        />
        <ProFormSelect
          name="tags"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.tags' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.tagsPlaceholder' })}
          mode="tags"
        />
        <ProFormTextArea
          name="notes"
          label={intl.formatMessage({ id: 'pages.passwordBook.form.notes' })}
          placeholder={intl.formatMessage({ id: 'pages.passwordBook.form.notesPlaceholder' })}
        />
      </ModalForm>

      <Drawer
        title={intl.formatMessage({ id: 'pages.passwordBook.detail.title' })}
        placement="right"
      ...
      <Modal
        title={intl.formatMessage({ id: 'pages.passwordBook.export.title' })}
        open={state.exportVisible}
        onCancel={() => set({ exportVisible: false })}
        footer={null}
        width={400}
      >
        <Button type="primary" icon={<DownloadOutlined />} block onClick={handleExport}>
          {intl.formatMessage({ id: 'pages.passwordBook.export.json' })}
        </Button>
      </Modal>
    </PageContainer>
  );
...
const CopyButton: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <Button type="text" size="small" icon={copied ? <CheckCircleOutlined /> : <CopyOutlined />} onClick={handleCopy}>
      {copied ? intl.formatMessage({ id: 'pages.passwordBook.copied' }) : ''}
    </Button>
  );
};
