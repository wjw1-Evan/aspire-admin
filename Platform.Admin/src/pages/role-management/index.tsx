...
import { Tag, Space, Card, Row, Col, Button, Input, Drawer, Descriptions, Popconfirm, Switch, Tree, Spin, Divider } from 'antd';
...
const RoleManagement: React.FC = () => {
  ...
  const columns: ProColumns<RoleWithStats>[] = [
    { title: intl.formatMessage({ id: 'pages.table.roleName' }), dataIndex: 'name', sorter: true, render: (dom: any, r) => <a onClick={() => set({ viewingRole: r, detailVisible: true })}>{dom}</a> },
  ...
  return (
    <PageContainer title={<Space><SafetyOutlined />{intl.formatMessage({ id: 'pages.roleManagement.title' })}</Space>} extra={<Space wrap><Button key="refresh" icon={<ReloadOutlined />} onClick={() => actionRef.current?.reload()}>{intl.formatMessage({ id: 'pages.button.refresh' })}</Button><Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => { set({ editingRole: null, formVisible: true }); setFormState(p => ({ ...p, checkedKeys: [], expandedKeys: [] })); }}>{intl.formatMessage({ id: 'pages.button.addRole' })}</Button></Space>}>
      {state.statistics && <Card style={{ marginBottom: 16 }}><Row gutter={[12, 12]}>
        {[{ key: 'totalRoles', title: intl.formatMessage({ id: 'pages.roleManagement.statistics.totalRoles' }), icon: <TeamOutlined />, color: '#1890ff' }, { key: 'activeRoles', title: intl.formatMessage({ id: 'pages.roleManagement.statistics.activeRoles' }), icon: <TeamOutlined />, color: '#52c41a' }, { key: 'totalUsers', title: intl.formatMessage({ id: 'pages.roleManagement.statistics.totalUsers' }), icon: <UserOutlined />, color: '#faad14' }, { key: 'totalMenus', title: intl.formatMessage({ id: 'pages.roleManagement.statistics.totalMenus' }), icon: <MenuOutlined />, color: '#722ed1' }].map(i => (
          <Col xs={24} sm={12} md={6} key={i.key}><StatCard title={i.title} value={state.statistics![i.key as keyof RoleStatistics]} icon={i.icon} color={i.color} /></Col>
        ))}
      </Row></Card>}
      ...
      <ProTable actionRef={actionRef} request={async (params: any) => {
        ...
        toolBarRender={() => [<Input.Search key="search" placeholder={intl.formatMessage({ id: 'pages.table.search' })} style={{ width: 200 }} allowClear value={state.searchText} onChange={(e) => set({ searchText: e.target.value })} onSearch={(v) => { set({ searchText: v }); actionRef.current?.reload(); }} />]}
      />
      ...
      <ModalForm key={state.editingRole?.id || 'create'}
        ...
        onFinish={async (values) => {
          ...
          if (res.success) { set({ formVisible: false, editingRole: null }); actionRef.current?.reload(); api.statistics().then(r => { if (r.success && r.data) set({ statistics: r.data }); }); }
          return res.success;
