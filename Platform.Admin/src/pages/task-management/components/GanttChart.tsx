...
const GanttChart: React.FC<GanttChartProps> = ({ projectId: initialProjectId }) => {
  ...
  }, [selectedProjectId, loadTasks, loadCriticalPath, loadAllTasks]);
  ...
  const flattenTasks = (taskList: TaskDto[]): TaskDto[] => {
    ...
    const traverse = (tasks: TaskDto[]) => {
      tasks.forEach((task) => {
        result.push(task);
        if (task.children?.length) traverse(task.children);
      });
    };
    traverse(taskList);
    return result;
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                    <Tag color={getStatusColor(task.status)} style={{ fontSize: 10, padding: '0 4px', margin: 0 }}>
                      {task.statusName}
                    </Tag>
                    {task.priorityName && (
                      <Tag
              ...
              {Array.from({ length: totalDays }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isToday && (
                      <div
                        style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: '#1890ff', marginTop: 2 }}
                      />
                    )}
                  </div>
                );
                ...
                <Tooltip
                  title={
                    <div>
                      ...
                      <div>
                        进度: {task.completionPercentage}% {isCritical && '| 关键路径'}
                      </div>
                    </div>
                  }
                >
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 400 }}>
                    {intl.formatMessage({ id: 'pages.taskManagement.timeUnit.totalDays' }, { count: totalDays })}
                  </span>
        ...
        {criticalPath.length > 0 && (
          <div
            style={{
              marginTop: 16,
              padding: '12px 16px',
              backgroundColor: 'linear-gradient(90deg, #fff7e6 0%, #fffbe6 100%)',
              borderRadius: 8,
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <strong style={{ color: '#d46b08' }}>
                {intl.formatMessage({ id: 'pages.project.gantt.criticalPath' })}
              </strong>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {(() => {
                const findCriticalTasks = (taskList: TaskDto[]): TaskDto[] => {
                  ...
                  const traverse = (tasks: TaskDto[]) => {
                    tasks.forEach((task) => {
                      if (task.id && criticalPath.includes(task.id)) result.push(task);
                      if (task.children?.length) traverse(task.children);
                    });
                  };
                  traverse(taskList);
