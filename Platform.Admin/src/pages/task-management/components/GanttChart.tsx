...
const GanttChart: React.FC<GanttChartProps> = ({ projectId: initialProjectId }) => {
  ...
  const renderGanttTable = () => {
    if (!tasks.length) return <div style={{ textAlign: 'center', padding: '40px' }}>{intl.formatMessage({ id: 'pages.projectManagement.gantt.noTasks' })}</div>;
    ...
    if (!allDates.length) {
      ...
      minDate = new Date(today); minDate.setDate(today.getDate() - 30);
    ...
    const renderTaskRow = (task: TaskDto, level = 0) => {
      ...
      let taskWidth = '0%', taskLeft = '0%', minTaskWidth = 'auto';
        ...
        <React.Fragment key={task.id}>
          <tr>
            <td style={{ border: '1px solid #ddd', padding: '8px', paddingLeft: `${8 + level * 20}px`, backgroundColor: '#fff', position: 'sticky', left: 0, zIndex: 1 }}>
              {level > 0 && '└─ '}<span style={{ fontWeight: level === 0 ? '500' : 'normal' }}>{task.taskName}</span>
            </td>
            <td style={{ border: '1px solid #ddd', padding: '8px', backgroundColor: '#fff', position: 'sticky', left: '200px', zIndex: 1 }}>
              <span style={{ color: task.completionPercentage === 100 ? '#52c41a' : task.completionPercentage > 0 ? '#1890ff' : '#8c8c8c' }}>{task.completionPercentage}%</span>
            </td>
            <td style={{ border: '1px solid #ddd', padding: '8px', position: 'relative', height: '40px', minWidth: `${Math.max(800, totalDays * 2)}px` }}>
              {hasTimeInfo ? (
                <div style={{ position: 'absolute', left: taskLeft, width: taskWidth, minWidth: minTaskWidth, height: '28px', backgroundColor: isCritical ? '#ff4d4f' : task.completionPercentage === 100 ? '#52c41a' : '#1890ff', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '11px', fontWeight: isCritical ? 'bold' : 'normal', boxShadow: isCritical ? '0 2px 4px rgba(255, 77, 79, 0.3)' : '0 1px 2px rgba(0, 0, 0, 0.1)', cursor: 'pointer', transition: 'opacity 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.opacity = '0.8'; }}
                  onMouseLeave={e => { e.currentTarget.style.opacity = '1'; }}
                  title={`${task.taskName}\n${dayjs(task.plannedStartTime).format('YYYY-MM-DD')} - ${dayjs(task.plannedEndTime).format('YYYY-MM-DD')}\n${Math.ceil((new Date(task.plannedEndTime!).getTime() - new Date(task.plannedStartTime!).getTime()) / (1000 * 60 * 60 * 24))}天 | ${task.completionPercentage}%${isCritical ? ' | 关键路径' : ''}`}>
                  {Math.ceil((new Date(task.plannedEndTime!).getTime() - new Date(task.plannedStartTime!).getTime()) / (1000 * 60 * 60 * 24)) >= 1 && <span>{Math.ceil((new Date(task.plannedEndTime!).getTime() - new Date(task.plannedStartTime!).getTime()) / (1000 * 60 * 60 * 24))}天</span>}
                </div>
              ) : <div style={{ padding: '4px 8px', color: '#999', fontSize: '12px', fontStyle: 'italic' }}>未设置时间</div>}
            </td>
          </tr>
    ...
    return (
      <div style={{ overflowX: 'auto', width: '100%' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ border: '1px solid #ddd', padding: '8px', width: '200px', textAlign: 'left', backgroundColor: '#fafafa', position: 'sticky', left: 0, zIndex: 10 }}>{intl.formatMessage({ id: 'pages.projectManagement.gantt.table.taskName' })}</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', width: '100px', backgroundColor: '#fafafa', position: 'sticky', left: '200px', zIndex: 10 }}>{intl.formatMessage({ id: 'pages.projectManagement.gantt.table.progress' })}</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', minWidth: `${Math.max(800, totalDays * 2)}px`, backgroundColor: '#fafafa' }}>{intl.formatMessage({ id: 'pages.projectManagement.gantt.table.timeline' })}</th>
            </tr>
            {totalDays <= 60 && (
              <tr>
                ...
                <th style={{ border: '1px solid #ddd', padding: '4px', backgroundColor: '#f5f5f5', fontSize: '11px', textAlign: 'center' }}>{dayjs(minDate).format('YYYY-MM-DD')} 至 {dayjs(maxDate).format('YYYY-MM-DD')} ({totalDays}天)</th>
              </tr>
            )}
          </thead>
        ...
        {criticalPath.length > 0 && (
          <div style={{ marginTop: 16, padding: 8, backgroundColor: '#fff7e6', borderRadius: 4 }}>
            ...
            {(() => {
              ...
              return findCriticalTasks(tasks).map(t => t.taskName).join(' → ');
            })()}
          </div>
        )}
      </ProCard>
