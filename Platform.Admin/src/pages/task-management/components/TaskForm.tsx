...
import { Modal, Form, Input, Select, DatePicker, InputNumber, Tag, message, Spin, Row, Col } from 'antd';
...
const TaskForm: React.FC<TaskFormProps> = ({
  ...
  useEffect(() => {
    if (open && task) {
      const assignedUserIds = Array.from(new Set([task.assignedTo, ...(task.participantIds as string[] | undefined) || []].filter(Boolean) as string[]));
      form.setFieldsValue({
        taskName: task.taskName, description: task.description,
        taskType: task.taskType ? [task.taskType] : undefined, priority: task.priority,
        assignedUserIds, plannedStartTime: task.plannedStartTime ? dayjs(task.plannedStartTime) : undefined,
        plannedEndTime: task.plannedEndTime ? dayjs(task.plannedEndTime) : undefined,
        estimatedDuration: task.estimatedDuration, participantIds: task.participantIds,
        tags: task.tags, remarks: task.remarks, projectId: task.projectId, parentTaskId: task.parentTaskId,
      });
    } else if (open) {
      form.resetFields();
      if (projectId) form.setFieldsValue({ projectId });
      if (parentTaskId) form.setFieldsValue({ parentTaskId });
    }
  }, [open, task, form]);
  ...
  const handleSubmit = async (values: TaskFormValues) => {
    ...
    try {
      ...
      if (task?.id) { await updateTask({ taskId: task.id, ...extendedData }); message.success('任务已更新'); }
      else { await createTask(extendedData); message.success('任务已创建'); }
      onSuccess?.();
    } catch (error) { console.error('提交表单错误:', error); message.error(task?.id ? '更新任务失败' : '创建任务失败'); }
  ...
  return (
    <Modal title={task?.id ? '编辑任务' : '创建任务'} open={open} onCancel={onClose || onCancel} onOk={() => form.submit()} width={800} confirmLoading={loading}>
      <Spin spinning={usersLoading}>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Row gutter={16}>
            ...
            <Col xs={24} sm={24} md={12}>
              <Form.Item label="任务类型" name="taskType" rules={[{ required: true, message: '请输入或选择任务类型' }]}>
                <Select mode="tags" maxCount={1} placeholder="请输入或选择任务类型" options={[
          ...
          <Row gutter={16}>
            <Col xs={24} sm={24} md={8}>
              <Form.Item label="优先级" name="priority" initialValue={TaskPriority.Medium}>
                <Select options={[
            ...
            <Col xs={24} sm={24} md={8}>
              <Form.Item label="分配给（可多选）" name="assignedUserIds">
                <Select mode="multiple" showSearch placeholder="请选择分配用户（可多选）" options={users.map(user => ({ label: user.name ? `${user.username} (${user.name})` : user.username, value: user.id }))} optionFilterProp="label" allowClear />
              </Form.Item>
            </Col>
            <Col xs={24} sm={24} md={8}>
              <Form.Item label="预计耗时（分钟）" name="estimatedDuration">
                <InputNumber min={0} placeholder="请输入预计耗时" style={{ width: '100%' }} />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col xs={24} sm={24} md={12}>
              <Form.Item label="计划开始时间" name="plannedStartTime">
                <DatePicker showTime format="YYYY-MM-DD HH:mm" placeholder="请选择计划开始时间" style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col xs={24} sm={24} md={12}>
              <Form.Item label="计划完成时间" name="plannedEndTime">
                <DatePicker showTime format="YYYY-MM-DD HH:mm" placeholder="请选择计划完成时间" style={{ width: '100%' }} />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item label="参与者" name="participantIds">
            <Select mode="multiple" placeholder="请选择参与者" showSearch options={users.map(user => ({ label: user.name ? `${user.username} (${user.name})` : user.username, value: user.id }))} optionFilterProp="label" />
          </Form.Item>
          <Form.Item label="标签" name="tags">
            <Select mode="tags" placeholder="请输入或选择标签" options={[
          ...
          <Form.Item label="所属项目" name="projectId" initialValue={projectId}>
            <Select placeholder="请选择项目（可选）" allowClear onChange={value => { if (value) loadTasks(value); else setTasks([]); form.setFieldsValue({ parentTaskId: undefined }); }} options={projects.map(p => ({ label: p.name, value: p.id }))} />
          </Form.Item>
          <Form.Item label="父任务" name="parentTaskId" initialValue={parentTaskId}>
            <Select placeholder="请选择父任务（可选）" allowClear disabled={!form.getFieldValue('projectId') && !projectId} options={tasks.filter(t => t.id !== task?.id).map(t => ({ label: t.taskName, value: t.id }))} />
          </Form.Item>
        </Form>
      </Spin>
    </Modal>
