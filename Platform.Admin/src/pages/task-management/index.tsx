...
// ==================== API ====================
const api = {
  list: (params: any) => request<ApiResponse<PagedResult<TaskDto>>>('/apiservice/api/task/query', { params }),
...
const TaskDetail: React.FC<{ id: string; onClose: () => void; open: boolean; isMobile: boolean }> = ({
  id,
  onClose,
  open,
  isMobile,
  ...
  }, [open, id, loadTaskDetail, loadExecutionLogs]);
  ...
  return (
    <Drawer
      title={intl.formatMessage({ id: 'pages.taskManagement.detail.title' })}
      placement="right"
      open={open}
      onClose={() => onClose()}
      size="large"
          ...
          <>
            ...
            >
              <ProDescriptions column={isMobile ? 1 : 2} size="small">
                ...
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.status' })}>
                  <Tag color={getTaskStatusColor(taskDetail.status)}>{taskDetail.statusName}</Tag>
                </ProDescriptions.Item>
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.priority' })}>
                  <Tag color={getTaskPriorityColor(taskDetail.priority)}>{taskDetail.priorityName}</Tag>
                </ProDescriptions.Item>
            ...
            >
              <ProDescriptions column={isMobile ? 1 : 2} size="small">
                ...
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.assignedTo' })}>
                  {taskDetail.assignedToName || '-'}
                </ProDescriptions.Item>
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.assignedAt' })}>
            ...
            >
              <ProDescriptions column={isMobile ? 1 : 2} size="small">
                ...
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.actualStart' })}>
                  {taskDetail.actualStartTime ? dayjs(taskDetail.actualStartTime).format('YYYY-MM-DD HH:mm:ss') : '-'}
                </ProDescriptions.Item>
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.actualEnd' })}>
                  {taskDetail.actualEndTime ? dayjs(taskDetail.actualEndTime).format('YYYY-MM-DD HH:mm:ss') : '-'}
                </ProDescriptions.Item>
                <ProDescriptions.Item
                  label={intl.formatMessage({ id: 'pages.taskManagement.table.estimatedDuration' })}
                >
                  {taskDetail.estimatedDuration
                    ? `${taskDetail.estimatedDuration} ${intl.formatMessage({ id: 'pages.taskManagement.timeUnit.minutes' })}`
                    : '-'}
                </ProDescriptions.Item>
                <ProDescriptions.Item label={intl.formatMessage({ id: 'pages.taskManagement.table.actualDuration' })}>
                  {taskDetail.actualDuration
                    ? `${taskDetail.actualDuration} ${intl.formatMessage({ id: 'pages.taskManagement.timeUnit.minutes' })}`
                    : '-'}
                </ProDescriptions.Item>
              </ProDescriptions>
            </ProCard>
            ...
            {taskDetail.participants && taskDetail.participants.length > 0 && (
              <ProCard
                title={intl.formatMessage({ id: 'pages.taskManagement.detail.participants' })}
                style={{ marginBottom: 16 }}
              >
                <Space wrap>
                  {taskDetail.participants.map((p) => (
                    <Tag key={p.userId}>{p.username}</Tag>
                  ))}
                </Space>
              </ProCard>
            )}
            ...
            {taskDetail.tags && taskDetail.tags.length > 0 && (
              <ProCard
                title={intl.formatMessage({ id: 'pages.taskManagement.table.tags' })}
                style={{ marginBottom: 16 }}
              >
                <Space wrap>
            ...
            {taskDetail.remarks && (
              <ProCard
                title={intl.formatMessage({ id: 'pages.taskManagement.table.remarks' })}
                style={{ marginBottom: 16 }}
              >
            ...
            <ProCard
              title={intl.formatMessage({ id: 'pages.taskManagement.detail.executionLogs' })}
              loading={logsLoading}
            >
                ...
                <Timeline
                  items={executionLogs.map((log) => ({
                    ...
                    content: (
                      <div>
                        <p>
                          <strong>{log.executedByName}</strong>{' '}
                          {intl.formatMessage(
                            { id: 'pages.taskManagement.detail.executedAt' },
                        ...
                        {log.message && (
                          <p>
                            {intl.formatMessage(
                              { id: 'pages.taskManagement.detail.message' },
                              { message: log.message },
                            )}
                          </p>
                        )}
                        {log.message && (
                          <p style={{ color: 'red' }}>
                            {intl.formatMessage({ id: 'pages.taskManagement.detail.error' }, { message: log.message })}
                          </p>
                        )}
                        <p>
                          {intl.formatMessage(
                            { id: 'pages.taskManagement.detail.progress' },
...
const TaskManagement: React.FC = () => {
  ...
  const [state, setState] = useState({
    ...
    editingTask: null as TaskDto | null,
    viewingTask: null as TaskDto | null,
    search: '',
  });
  ...
  useEffect(() => {
    loadStatistics();
    getProjectList({ page: 1, pageSize: 100 }).then((r) => {
      if (r.success && r.data) setProjects(r.data.queryable?.map((p) => ({ id: p.id!, name: p.name })) || []);
    });
  }, [loadStatistics]);
  ...
  const columns: ProColumns<TaskDto>[] = useMemo(
    () => [
      {
        title: intl.formatMessage({ id: 'pages.taskManagement.table.name' }),
        dataIndex: 'taskName',
        key: 'taskName',
        width: 200,
        sorter: true,
        render: (_: unknown, r: TaskDto) => (
          <a onClick={() => set({ viewingTask: r, detailVisible: true })}>{r.taskName}</a>
        ),
      },
      {
        title: intl.formatMessage({ id: 'pages.taskManagement.table.type' }),
        dataIndex: 'taskType',
        key: 'taskType',
        width: 100,
        sorter: true,
        render: (_: unknown, r: TaskDto) => (r.taskType ? <Tag>{r.taskType}</Tag> : '-'),
      },
      {
        title: intl.formatMessage({ id: 'pages.taskManagement.table.projectName' }),
        dataIndex: 'projectName',
        key: 'projectName',
        width: 150,
        sorter: true,
        render: (_: unknown, r: TaskDto) => r.projectName || '-',
      },
      {
        ...
        width: 100,
        sorter: true,
        render: (_: unknown, r: TaskDto) => <Tag color={getTaskStatusColor(r.status)}>{r.statusName}</Tag>,
      },
      {
        ...
        width: 80,
        sorter: true,
        render: (_: unknown, r: TaskDto) => <Tag color={getTaskPriorityColor(r.priority)}>{r.priorityName}</Tag>,
      },
      {
        ...
        render: (_: unknown, r: TaskDto) => (
          <Progress
            ...
            size="small"
            status={
              r.completionPercentage === 100 ? 'success' : r.status === TaskStatusEnum.Failed ? 'exception' : 'active'
            }
          />
        ),
      },
      {
        ...
        width: 100,
        sorter: true,
        render: (_: unknown, r: TaskDto) => r.assignedToName || '-',
      },
      {
        ...
        width: 100,
        sorter: true,
        render: (_: unknown, r: TaskDto) => r.createdByName || '-',
      },
      {
        title: intl.formatMessage({ id: 'pages.taskManagement.table.plannedEnd' }),
        dataIndex: 'plannedEndTime',
        key: 'plannedEndTime',
        width: 150,
        sorter: true,
        render: (_: unknown, r: TaskDto) =>
          r.plannedEndTime ? dayjs(r.plannedEndTime).format('YYYY-MM-DD HH:mm') : '-',
      },
      {
        ...
        width: 150,
        sorter: true,
        render: (_: unknown, r: TaskDto) => (r.createdAt ? dayjs(r.createdAt).format('YYYY-MM-DD HH:mm') : '-'),
      },
      {
        ...
        fixed: 'right',
        width: 200,
        render: (_: React.ReactNode, r: TaskDto) => {
            ...
            <Space size={4}>
                ...
                <>
                  ...
                  <Popconfirm
                    title={intl.formatMessage(
                      { id: 'pages.taskManagement.message.confirmDeleteWithName' },
                      { name: r.taskName },
                    )}
                    onConfirm={() =>
                      api.delete(r.id || '').then((res) => {
                        if (res.success) {
                          message.success(intl.formatMessage({ id: 'pages.taskManagement.message.deleteSuccess' }));
                          actionRef.current?.reload();
                          loadStatistics();
                        } else {
                          message.error(getErrorMessage(res, 'pages.taskManagement.message.deleteFailed'));
                        }
                      })
                    }
                  >
              ...
              {isAssignee && r.status !== TaskStatusEnum.Completed && r.status !== TaskStatusEnum.Cancelled && (
                <Button
                  ...
                  onClick={() => {
                    set({ viewingTask: r, executionVisible: true });
                  }}
                >
    ...
    [intl, message, loadStatistics, currentUserId, set],
  );
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        headerTitle={
          <Space size={24}>
            ...
            <Space size={12}>
              <Tag color="blue">
                {intl.formatMessage({ id: 'pages.taskManagement.statistics.totalTasks' })}{' '}
                {state.statistics?.totalTasks || 0}
              </Tag>
              <Tag color="orange">
                {intl.formatMessage({ id: 'pages.taskManagement.statistics.inProgressTasks' })}{' '}
                {state.statistics?.inProgressTasks || 0}
              </Tag>
              <Tag color="green">
                {intl.formatMessage({ id: 'pages.taskManagement.statistics.completedTasks' })}{' '}
                {state.statistics?.completedTasks || 0}
              </Tag>
              <Tag color="purple">
                {intl.formatMessage({ id: 'pages.taskManagement.statistics.completionRate' })}{' '}
                {state.statistics?.completionRate ? `${state.statistics.completionRate.toFixed(1)}%` : '0%'}
              </Tag>
            </Space>
          </Space>
        }
      ...
      <TaskDetail
        id={state.viewingTask?.id || ''}
        open={state.detailVisible}
        onClose={() => set({ detailVisible: false, viewingTask: null })}
        isMobile={isMobile}
      />

      <TaskExecutionPanel
        open={state.executionVisible}
        task={state.viewingTask}
        onClose={() => set({ executionVisible: false, viewingTask: null })}
        onSuccess={() => {
          set({ executionVisible: false, viewingTask: null });
          actionRef.current?.reload();
          loadStatistics();
        }}
      />
