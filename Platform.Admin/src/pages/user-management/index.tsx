...
const api = {
  ...
  del: (id: string, reason?: string) =>
    request<ApiResponse<unknown>>(`/apiservice/api/users/${id}`, {
      method: 'DELETE',
  ...
  joinReqs: (cid: string, params?: any) =>
    request<ApiResponse<PagedResult<JoinReq>>>(`/apiservice/api/company/${cid}/join-requests`, {
      method: 'GET',
      params,
    }),
  approveJoin: (id: string) =>
    request<ApiResponse<unknown>>(`/apiservice/api/company/join-requests/${id}/approve`, { method: 'POST', data: {} }),
  rejectJoin: (id: string, data: { rejectReason: string }) =>
    request<ApiResponse<unknown>>(`/apiservice/api/company/join-requests/${id}/reject`, { method: 'POST', data }),
  unifiedList: async (params: any, companyId?: string) => {
    ...
    const [usersRes, joinReqsRes] = await Promise.all([
      request<ApiResponse<PagedResult<UnifiedUser>>>('/apiservice/api/users/list', {
        params: { current, pageSize, search, sort, filter },
      }),
      companyId
        ? request<ApiResponse<PagedResult<JoinReq>>>(`/apiservice/api/company/${companyId}/join-requests`, {
            params: { page: current, pageSize },
          })
        : Promise.resolve({ success: true, data: { queryable: [], rowCount: 0 } }),
    ]);
...
const UserManagement: React.FC = () => {
  ...
  const _handleToggle = useCallback(
    async (user: UnifiedUser) => {
      ...
      if (res.success) {
        message.success(
          user.isActive
            ? intl.formatMessage({ id: 'pages.userManagement.message.deactivated' })
            : intl.formatMessage({ id: 'pages.userManagement.message.activated' }),
        );
        loadStatistics();
    ...
    (userId: string) => {
      ...
      modal.confirm({
        ...
        content: (
          <Input.TextArea
            rows={3}
            placeholder={intl.formatMessage({ id: 'pages.userManagement.form.reasonPlaceholder' })}
            onChange={(e) => {
        ...
        okText: intl.formatMessage({ id: 'pages.userManagement.action.confirmRemove' }),
        cancelText: intl.formatMessage({ id: 'pages.action.cancel' }),
        okType: 'danger',
        onOk: async () => {
          const res = await api.del(userId, reason);
          if (res.success) {
            message.success(intl.formatMessage({ id: 'pages.userManagement.message.removeSuccess' }));
            loadStatistics();
            actionRef.current?.reload();
          } else {
            message.error(getErrorMessage(res, 'pages.userManagement.message.removeFailed'));
          }
        },
      });
    },
  ...
  const handleApprove = useCallback(
    async (id: string) => {
      ...
      if (res.success) {
        ...
        message.error(getErrorMessage(res, 'pages.userManagement.message.approveFailed'));
      }
    },
    [message, intl, loadStatistics],
  );
  ...
  const confirmReject = useCallback(async () => {
    if (!state.rejectReason.trim()) {
      message.error(intl.formatMessage({ id: 'pages.userManagement.message.rejectReasonRequired' }));
      return;
    }
    ...
    if (res.success) {
      ...
      message.error(getErrorMessage(res, 'pages.userManagement.message.rejectFailed'));
    }
  }, [state.rejectId, state.rejectReason, message, set, intl, loadStatistics]);

  const columns: ProColumns<UnifiedUser>[] = useMemo(
    () => [
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.username' }),
        dataIndex: 'username',
        key: 'username',
        sorter: true,
        render: (dom, r) => (
          <Space>
            ...
            {state.currentCompany?.createdBy === r.id && (
              <Tag icon={<CrownOutlined />} color="gold">
                {intl.formatMessage({ id: 'pages.userManagement.tag.creator' })}
              </Tag>
            )}
          </Space>
        ),
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.name' }),
        dataIndex: 'name',
        key: 'name',
        ellipsis: true,
        sorter: true,
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.email' }),
        dataIndex: 'email',
        key: 'email',
        ellipsis: true,
        responsive: ['md'],
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.phoneNumber' }),
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.joinStatus' }),
        dataIndex: 'joinStatus',
        key: 'joinStatus',
        render: (_, r) => {
          if (!r.joinStatus)
            return <Tag color="green">{intl.formatMessage({ id: 'pages.userManagement.joinStatus.joined' })}</Tag>;
          const statusMap = {
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.roles' }),
        dataIndex: 'roleIds',
        responsive: ['md'],
        render: (_, r) =>
          !r.roleIds?.length ? (
            <Tag>{intl.formatMessage({ id: 'pages.userManagement.tag.unassigned' })}</Tag>
          ) : (
            <Space wrap>
              {r.roleIds.map((id) => (
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.status' }),
        dataIndex: 'isActive',
        key: 'isActive',
        render: (_, r) => {
          ...
          return (
            <Badge
              ...
              text={
                r.isActive
                  ? intl.formatMessage({ id: 'pages.userManagement.status.active' })
                  : intl.formatMessage({ id: 'pages.userManagement.status.inactive' })
              }
            />
          );
        },
      ...
      {
        title: intl.formatMessage({ id: 'pages.userManagement.table.createdAt' }),
        dataIndex: 'createdAt',
        key: 'createdAt',
        sorter: true,
        valueType: 'dateTime',
            ...
            {r.joinStatus === 'pending' ? (
              <>
                <Button type="link" size="small" icon={<CheckOutlined />} onClick={() => handleApprove(r.id)}>
                  {intl.formatMessage({ id: 'pages.userManagement.action.approve' })}
                </Button>
                <Button type="link" size="small" danger icon={<CloseOutlined />} onClick={() => handleReject(r.id)}>
                  {intl.formatMessage({ id: 'pages.userManagement.action.reject' })}
                </Button>
              </>
            ) : (
              <>
                <Button
                  ...
                  size="small"
                  icon={<EditOutlined />}
                  onClick={() => set({ formVisible: true, editingUser: r })}
                >
                  {intl.formatMessage({ id: 'pages.action.edit' })}
                </Button>
                <Popconfirm
                  title={intl.formatMessage(
                    { id: 'pages.userManagement.message.confirmRemoveUserWithName' },
                    { name: r.name },
                  )}
                  onConfirm={() => promptDelete(r.id)}
                >
  ...
  return (
    <PageContainer>
      <ProTable
          ...
          headerTitle={
            <Space size={24}>
              ...
              <Space size={12}>
                <Tag color="blue">
                  {intl.formatMessage({ id: 'pages.userManagement.statistics.totalUsers' })}{' '}
                  {state.statistics?.totalUsers || 0}
                </Tag>
                <Tag color="green">
                  {intl.formatMessage({ id: 'pages.userManagement.statistics.activeUsers' })}{' '}
                  {state.statistics?.activeUsers || 0}
                </Tag>
                <Tag color="orange">
                  {intl.formatMessage({ id: 'pages.userManagement.statistics.adminUsers' })}{' '}
                  {state.statistics?.adminUsers || 0}
                </Tag>
                <Tag color="purple">
                  {intl.formatMessage({ id: 'pages.userManagement.statistics.newUsersThisMonth' })}{' '}
                  {state.statistics?.newUsersThisMonth || 0}
                </Tag>
                <Tag color="cyan">
                  {intl.formatMessage({ id: 'pages.userManagement.statistics.pendingUsers' })}{' '}
                  {state.statistics?.pendingUsers || 0}
                </Tag>
              </Space>
            </Space>
          }
      ...
      <ModalForm
        ...
        onOpenChange={(open) => {
          if (!open) set({ formVisible: false, editingUser: null });
        }}
        initialValues={
          state.editingUser
            ? {
                roleIds: state.editingUser.roleIds || [],
                isActive: state.editingUser.isActive,
                remark: state.editingUser.remark,
              }
            : { isActive: true }
        }
      ...
      >
        <ProFormSelect
          ...
          mode="multiple"
          placeholder={intl.formatMessage({ id: 'pages.userManagement.form.rolesPlaceholder' })}
          options={roleOptions}
          rules={[{ required: true, message: intl.formatMessage({ id: 'pages.userManagement.form.rolesRequired' }) }]}
        />
        <ProFormSwitch
          name="isActive"
          label={intl.formatMessage({ id: 'pages.userManagement.form.isActive' })}
          checkedChildren={intl.formatMessage({ id: 'pages.userManagement.form.active' })}
          unCheckedChildren={intl.formatMessage({ id: 'pages.userManagement.form.inactive' })}
        />
        <ProFormTextArea name="remark" label={intl.formatMessage({ id: 'pages.userManagement.form.remark' })} />
      </ModalForm>

      <Drawer
        title={intl.formatMessage({ id: 'pages.userManagement.detail.title' })}
        placement="right"
        open={state.detailVisible}
        onClose={() => set({ detailVisible: false, viewingUser: null })}
        size="large"
        destroyOnClose
      >
        <React.Suspense
