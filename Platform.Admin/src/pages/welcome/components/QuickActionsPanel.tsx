  ...
  return (
    <ProCard
      ...
      extra={
        hasOverflow ? (
          ...
          >
            {collapsed
              ? intl.formatMessage({ id: 'pages.welcome.quickActions.expand' })
              : intl.formatMessage({ id: 'pages.welcome.quickActions.collapse' })}
          </Button>
        ) : null
      }
    ...
    >
      {quickActionMenus.length > 0 ? (
        ...
        >
          <Row gutter={[16, 16]}>
            {quickActionMenus.map((menu) => {
              ...
              const menuDescription = intl.formatMessage({ id: `${localeKey}.desc` });

              return (
                <Col key={menu.path || menu.id} xs={24} sm={12} md={8} lg={6} xl={4}>
                  <QuickAction
                    title={menuTitle}
        ...
        <Alert title={intl.formatMessage({ id: 'pages.welcome.quickActions.empty' }, {})} type="info" showIcon />
      )}
    </ProCard>
  );
};
