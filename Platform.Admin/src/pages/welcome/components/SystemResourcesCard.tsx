    ...
    >
            ...
            >
              <div style={{ fontSize: '12px', color: 'var(--ant-color-text-tertiary)', textAlign: 'center' }}>
                {intl.formatMessage(
                  ...
                  {
                    used: (
                      (systemResources.memory?.totalMemoryMB || 0) - (systemResources.memory?.availableMemoryMB || 0)
                    ).toFixed(2),
      ...
      {systemResources?.system && (
        ...
        >
          <Title
            level={5}
            style={{ marginBottom: 12, fontSize: '14px', display: 'flex', alignItems: 'center', gap: 8 }}
          >
          ...
          <Row gutter={[16, 12]}>
            {[
              ...
              {
                label: 'architecture',
                value: systemResources.system?.is64BitOperatingSystem
                  ? intl.formatMessage({ id: 'pages.welcome.systemDetails.bit64' })
                  : intl.formatMessage({ id: 'pages.welcome.systemDetails.bit32' }),
              },
              ...
              <Col key={idx} xs={24} sm={12} md={item.large ? 12 : 6}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <Text type="secondary" style={{ fontSize: '12px' }}>
                    {intl.formatMessage({ id: `pages.welcome.systemDetails.${item.label}` })}
                  </Text>
      ...
      {systemResources?.database && (
        ...
        >
          ...
          >
            ...
            <Tag
              color={systemResources.database?.status === 'Connected' ? 'success' : 'default'}
              style={{ marginLeft: 8 }}
            >
          ...
          <Row gutter={[16, 12]}>
            {[
              ...
              <Col key={idx} xs={24} sm={12} md={6}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <Text type="secondary" style={{ fontSize: '12px' }}>
                    {intl.formatMessage({ id: `pages.welcome.databaseDetails.${item.label}` })}
                  </Text>
