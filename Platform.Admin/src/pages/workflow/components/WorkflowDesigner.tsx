...
import ReactFlow, {
  ...
  useNodesState, useEdgesState, Connection, MarkerType, Position,
} from 'reactflow';
...
import { SaveOutlined, CheckCircleOutlined, BranchesOutlined } from '@ant-design/icons';
import { useIntl } from '@umijs/max';
import { useMessage } from '@/hooks/useMessage';
import { useModal } from '@/hooks/useModal';
import type { WorkflowGraph, NodeConfig, ApproverRule } from '@/services/workflow/api';
import { ApproverType, FormTarget } from '@/services/workflow/api';
...
const useWorkflowVariables = (nodes: Node[], forms: FormDefinition[]) => useMemo(() => {
  const vars = [
  ...
  nodes.forEach(node => {
    ...
    if (nodeType && nodeType !== 'start') {
      vars.push({ label: `节点输出: ${label}`, value: `nodes.${node.id}.output` });
    }
    const formDefId = node.data?.config?.form?.formDefinitionId;
    if (formDefId) {
      const formDef = forms.find(f => f.id === formDefId);
      formDef?.fields?.forEach(field => {
        vars.push({ label: `表单字段: ${label} -> ${field.label}`, value: field.dataKey });
      });
    }
  });
  return vars;
...
const deepCleanIdFields = (obj: any): any => {
  ...
  if (Array.isArray(obj)) return obj.map(deepCleanIdFields);
  const newObj: any = {};
  Object.keys(obj).forEach((key) => {
    const value = obj[key];
    if (typeof value === 'string' && value.trim() === '' && (key.endsWith('Id') || key === 'id')) return;
    if (Array.isArray(value) && (key === 'userIds' || key === 'roleIds' || key === 'departmentIds')) {
      const cleaned = value.filter((v: string) => typeof v === 'string' && v.trim() !== '');
      if (cleaned.length > 0) newObj[key] = cleaned;
      return;
    }
...
const WorkflowDesigner: React.FC<WorkflowDesignerProps> = ({
  ...
  const defaultEdgeOptions = useMemo(() => ({
    type: 'workflowEdge', animated: true,
    ...
    const loadData = async () => {
      try {
        const [usersResponse, rolesResponse, formsResponse] = await Promise.all([
          ...
          getAllRoles(),
          getFormList({ pageSize: 200, page: 1 }),
        ]);
  ...
  useEffect(() => {
    if (graph && !hasInitializedRef.current) {
      const initialNodes: Node[] = graph.nodes.map((node) => {
        ...
        return {
          ...
          data: {
            ...
            nodeType: node.type, config: node.data?.config, jumpLabel,
          },
        };
      });
      const initialEdges: Edge[] = graph.edges.map((edge) => ({
        ...
        type: 'workflowEdge', label: edge.label,
      ...
      setNodes([{
        id: 'start', type: 'workflowNode',
        position: { x: 250, y: 50 }, deletable: false,
  ...
  const onConnect = useCallback((params: Connection) => {
    const newEdge: Edge = {
      id: `edge-${Date.now()}`, source: params.source!, target: params.target!,
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const id = `${type}-${Date.now()}`;
    const newNode: Node = {
      id, type: 'workflowNode', position, deletable: type !== 'start',
      data: {
        label: `新${typeLabels[type as keyof typeof typeLabels] || type}`,
        typeLabel: typeLabels[type as keyof typeof typeLabels] || type,
        nodeType: type, config: {},
  ...
  const onNodeClick = useCallback((_event: React.MouseEvent, node: Node) => {
    ...
    const mapApprovalType = (type: any) => {
      ...
      if (t === 'all') return 0; if (t === 'any') return 1; if (t === 'sequential') return 2;
      return 0;
    };
    const mapApproverType = (type: any) => {
      if (typeof type === 'number') return type;
      const t = String(type).toLowerCase();
      if (t === 'user') return 0; if (t === 'role') return 1; if (t === 'department') return 2; if (t === 'formfield') return 3;
      return 0;
    };
    configForm.resetFields();
    configForm.setFieldsValue({
      label: node.data.label || '', nodeType: node.data.nodeType,
  ...
  const handleSave = useCallback(() => {
    ...
    const workflowGraph: WorkflowGraph = {
      nodes: nodes.map((node) => ({
        id: node.id, type: node.data.nodeType as any,
        data: { nodeType: node.data.nodeType, label: node.data.label || '', config: deepCleanIdFields(node.data.config || {}) },
        position: { x: node.position.x, y: node.position.y },
      })),
      edges: edges.map((edge) => ({
        id: edge.id, source: edge.source, target: edge.target,
        sourceHandle: edge.sourceHandle ?? undefined, targetHandle: edge.targetHandle ?? undefined,
        label: edge.label as string, data: { condition: edge.data?.condition },
      })),
    };
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', background: '#fcfdfe', borderRadius: 16, overflow: 'hidden', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05), 0 8px 10px -6px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
      <WorkflowSidebar
        ...
        readOnly={readOnly}
      />
      <div style={{ flex: 1, position: 'relative', background: '#fcfdfe' }} ref={reactFlowWrapper}>
        ...
        <ReactFlow
          ...
          onNodeClick={onNodeClick} onInit={setReactFlowInstance}
        ...
        >
          ...
          <MiniMap
            ...
            maskStrokeColor="#3b82f6" style={{ borderRadius: 12 }}
          />
        </ReactFlow>
      </div>
      <NodeConfigDrawer
        visible={configDrawerVisible} onClose={() => setConfigDrawerVisible(false)}
        selectedNode={selectedNode} configForm={configForm} onSaveConfig={handleSaveConfig}
        onDeleteNode={handleDeleteNode} readOnly={readOnly}
        users={users} roles={roles} forms={forms}
        availableVariables={availableVariables} allNodes={nodes}
      />
    </div>
  );
};
