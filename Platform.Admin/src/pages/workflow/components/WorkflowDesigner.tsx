...
import type { ApproverRule, NodeConfig, WorkflowGraph } from '@/services/workflow/api';
import { ApproverType, FormTarget } from '@/services/workflow/api';
import NodeConfigDrawer from './NodeConfigDrawer';
import { edgeTypes, NODE_CONFIGS, nodeTypes } from './WorkflowDesignerConstants';
...
const WorkflowDesigner: React.FC<WorkflowDesignerProps> = React.memo(({ graph, onSave, onClose, readOnly = false }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (rolesResponse.success && rolesResponse.data)
          setRoles((rolesResponse.data.queryable || []).filter((r: Role) => r.isActive));
  ...
  }, [graph, typeLabels, setNodes, setEdges, nodes.length]);
  ...
  const onNodeClick = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      ...
      configForm.setFieldsValue({
        ...
        approvers:
          config.approval?.approvers?.map((r: ApproverRule) => {
        ...
        approveNextNodeId:
          node.data?.nodeType === 'approval'
            ? edges.find(
                (e) => e.source === node.id && (e.data?.condition === 'approve' || e.sourceHandle === 'approve'),
              )?.target
            : undefined,
        rejectNextNodeId:
          node.data?.nodeType === 'approval'
            ? edges.find((e) => e.source === node.id && (e.data?.condition === 'reject' || e.sourceHandle === 'reject'))
                ?.target
            : undefined,
      });
    },
  ...
  const formatApprovers = useCallback((rules: any[]): ApproverRule[] => {
    return (rules || []).flatMap((rule: any) => {
      if (rule.type === 0) {
        ...
        return uids
          .filter((id: string) => id)
          .map((uid: string) => ({ type: ApproverType.User, userId: uid }) as ApproverRule);
      }
      if (rule.type === 1) {
        const rids = Array.isArray(rule.roleIds) ? rule.roleIds : rule.roleId ? [rule.roleId] : [];
        return rids
          .filter((id: string) => id)
          .map((rid: string) => ({ type: ApproverType.Role, roleId: rid }) as ApproverRule);
      }
  ...
  const buildApprovalConfig = useCallback(
    (values: any): NodeConfig => ({
      approval: {
  ...
  const buildNodeEdges = useCallback(
      ...
      if (config.condition?.branches) {
        config.condition.branches.forEach((branch: any) => {
          if (branch.targetNodeId)
            addEdgeIfTarget(branch.targetNodeId, { sourceHandle: branch.id, label: branch.label });
        });
        if (config.condition.defaultNodeId)
          addEdgeIfTarget(config.condition.defaultNodeId, { sourceHandle: 'default', label: '默认' });
      } else if (values.nodeType === 'approval') {
        addEdgeIfTarget(values.approveNextNodeId, {
          sourceHandle: 'approve',
          label: '通过',
          data: { condition: 'approve' },
        });
        addEdgeIfTarget(values.rejectNextNodeId, {
          sourceHandle: 'reject',
          label: '拒绝',
    ...
    configForm
      ...
      .then((values) => {
        const updatedNodes = nodes.map((node: Node): Node => {
          ...
          const jumpLabel =
            values.nodeType === 'condition' && config.condition?.branches
  ...
  const handleDeleteNode = useCallback(() => {
    ...
    confirm({
      ...
      okText: intl.formatMessage({ id: 'pages.workflow.designer.delete' }),
      cancelText: intl.formatMessage({ id: 'pages.workflow.designer.cancel' }),
  ...
  }, [selectedNode, setNodes, setEdges, confirm, message, intl.formatMessage]);
      ...
      <div style={{ flex: 1, position: 'relative', background: '#fcfdfe' }} ref={reactFlowWrapper}>
        <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 10, display: 'flex', gap: 8 }}>
          ...
          <Button
            ...
            onClick={() => {
              if (validateWorkflow())
                message.success(intl.formatMessage({ id: 'pages.workflow.designer.messageValidationPassed' }));
            }}
          >
            验证
          </Button>
