...
const FormDefinitionManagement: React.FC = () => {
  ...
  }, [state.designerVisible, state.editingForm?.id, set]);

  const columns: ProColumns<FormDefinition>[] = [
    {
      title: intl.formatMessage({ id: 'pages.forms.table.name' }),
      dataIndex: 'name',
      key: 'name',
      ellipsis: true,
      sorter: true,
    ...
    {
      title: intl.formatMessage({ id: 'pages.forms.table.version' }),
      dataIndex: 'version',
      key: 'version',
      valueType: 'digit',
      width: 80,
    ...
    {
      title: intl.formatMessage({ id: 'pages.forms.table.fieldCount' }),
      dataIndex: 'fields',
      key: 'fields',
      valueType: 'digit',
      width: 80,
      render: (_, r) => r.fields?.length || 0,
    },
    {
      title: intl.formatMessage({ id: 'pages.forms.table.enabled' }),
      dataIndex: 'isActive',
      key: 'isActive',
      render: (_, r) => (
        <Tag color={r.isActive ? 'success' : 'default'}>
          {r.isActive
            ? intl.formatMessage({ id: 'pages.forms.status.yes' })
            : intl.formatMessage({ id: 'pages.forms.status.no' })}
        </Tag>
      ),
    },
    {
      title: intl.formatMessage({ id: 'pages.forms.table.action' }),
      key: 'action',
      valueType: 'option',
      fixed: 'right',
      width: 250,
      render: (_, r) => (
        <Space size={4}>
          <Button
            ...
            size="small"
            icon={<EyeOutlined />}
            onClick={() => set({ viewingFormId: r.id!, versionsDrawerVisible: true })}
          >
            {intl.formatMessage({ id: 'pages.forms.action.view' })}
          </Button>
          <Button
            type="link"
            size="small"
          ...
          >
            {intl.formatMessage({ id: 'pages.forms.action.edit' })}
          </Button>
          <Popconfirm
            title={intl.formatMessage({ id: 'pages.forms.message.confirmDelete' }, { name: r.name })}
            onConfirm={async () => {
          ...
          >
            <Button type="link" size="small" danger icon={<DeleteOutlined />}>
              {intl.formatMessage({ id: 'pages.forms.action.delete' })}
            </Button>
          </Popconfirm>
        </Space>
      ),
  ...
  return (
    <PageContainer>
      ...
      <ProTable
        ...
        headerTitle={
          <Space size={24}>
            <Space>
              <PartitionOutlined />
              {intl.formatMessage({ id: 'pages.forms.title' })}
            </Space>
            <Space size={12}>
              <Tag color="blue">
                {intl.formatMessage({ id: 'pages.forms.statistics.total' })} {state.statistics?.totalForms || 0}
              </Tag>
              <Tag color="green">
                {intl.formatMessage({ id: 'pages.forms.statistics.active' })} {state.statistics?.activeForms || 0}
              </Tag>
            </Space>
          </Space>
        }
        ...
        toolBarRender={() => [
          ...
          <Button
            ...
            onClick={() =>
              set({
                editingForm: {
                  name: intl.formatMessage({ id: 'pages.forms.form.newForm' }),
                  version: 1,
                  isActive: true,
                  fields: [],
                },
          ...
          >
            {intl.formatMessage({ id: 'pages.forms.action.create' })}
          </Button>,
        ]}
      />
      ...
      <Drawer
        title={
          state.editingForm?.id
            ? `${intl.formatMessage({ id: 'pages.forms.form.editForm' })}: ${state.editingForm.name}`
            : intl.formatMessage({ id: 'pages.forms.form.createForm' })
        }
        size="100%"
        open={state.designerVisible}
        onClose={() => set({ designerVisible: false, editingForm: null })}
      >
        {state.designerVisible && (
          <FormDesigner
            key={state.editingForm?.id || 'new'}
            form={
              state.editingForm || {
                id: '',
                name: intl.formatMessage({ id: 'pages.forms.form.newForm' }),
      ...
      <Drawer
        title={`${intl.formatMessage({ id: 'pages.forms.detail.versionHistory' })}: ${state.viewingFormId ? state.versions.find((v) => v.formDefinitionId === state.viewingFormId)?.name : ''}`}
        size={800}
        open={state.versionsDrawerVisible}
        onClose={() =>
          set({
            versionsDrawerVisible: false,
            viewingFormId: null,
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500, marginBottom: 8 }}>
              {intl.formatMessage({ id: 'pages.forms.detail.versionHistory' })}
            </div>
            <div>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: '#666' }}>
                      ...
                      {item.isActive
                        ? intl.formatMessage({ id: 'pages.forms.status.enabled' })
                        : intl.formatMessage({ id: 'pages.forms.status.disabled' })}
                    </div>
                  </div>
                  <Button
                    type="link"
                  ...
                  >
                    {intl.formatMessage({ id: 'pages.forms.action.rollback' })}
                  </Button>
                </div>
              ))}
            </div>
          ...
          <div style={{ flex: 2, borderLeft: '1px solid #f0f0f0', paddingLeft: 24 }}>
            <div style={{ fontWeight: 500, marginBottom: 8 }}>
              {intl.formatMessage({ id: 'pages.forms.detail.preview' })}{' '}
              {state.previewVersionId
                ? ` v${state.versions.find((v) => v.id === state.previewVersionId)?.version}`
                : ''}
            </div>
              ...
              <Form layout="vertical">
                {state.previewFields.map((field: any) => (
                  <Form.Item key={field.id} label={field.label} required={field.required}>
                    ...
                    {field.type === 'TextArea' && (
                      <Input.TextArea rows={2} placeholder={field.placeholder} defaultValue={field.defaultValue} />
                    )}
                    {field.type === 'Number' && (
