...
export class PasswordEncryption {
  ...
  private static async getValidPublicKey(): Promise<string> {
    const now = Date.now();
    if (PasswordEncryption.publicKey && now - PasswordEncryption.lastFetchTime < PasswordEncryption.CACHE_DURATION) {
      return PasswordEncryption.publicKey;
    }
    ...
    try {
      ...
      if (response.success && key && key.length === 130 && key.startsWith('04')) {
        PasswordEncryption.publicKey = key;
        PasswordEncryption.lastFetchTime = now;
        return PasswordEncryption.publicKey;
      }
      throw new Error('无效的 SM2 公钥格式（应为 04 开头的 130 位非压缩点）');
    } catch (error) {
      console.error('SM2 公钥获取异常:', error);
      PasswordEncryption.publicKey = null;
      PasswordEncryption.lastFetchTime = 0;
      throw error;
    }
  }
  ...
  public static async encrypt(password: string): Promise<string> {
    ...
    try {
      const keyHex = await PasswordEncryption.getValidPublicKey();
