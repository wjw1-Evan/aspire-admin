...
public class ChatAiService : IChatAiService
{
    ...
    /// 翻译文本到目标语言
    /// </summary>
    public async Task<string> TranslateTextAsync(string text, string targetLocale, string? sourceText, string userId, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(text)) return text;
        if (string.IsNullOrWhiteSpace(targetLocale)) return text;
        ...
        try
        {
            ...
            if (xiaokeConfig == null || !xiaokeConfig.IsEnabled)
            {
                _logger.LogWarning("【翻译】小科配置不可用，跳过翻译");
                return text;
            }
            ...
            var targetLang = localeDisplayNames.TryGetValue(targetLocale, out var name) ? name : targetLocale;
            var sourceLang = "英语";

            var prompt = $@"你是专业翻译员。将以下文本翻译成{targetLang}。
规则：
1. 保持与原文相同的语气和风格
2. 保留占位符变量如 {{0}}、{{name}} 等
3. 如果是UI标签，保持简洁
...
翻译：";
            ...
            var messages = new List<Microsoft.Extensions.AI.ChatMessage>
            {
                new Microsoft.Extensions.AI.ChatMessage(ChatRole.System, prompt)
            };

            _logger.LogInformation("【翻译】开始翻译 | 目标语言={TargetLocale} | 文本长度={Length}", targetLocale, text.Length);

            var result = await _openAiClient.GetResponseAsync(messages, chatOptions, cancellationToken);
            var translatedText = result?.Text?.Trim() ?? text;

            _logger.LogInformation("【翻译】翻译完成 | 结果长度={Length}", translatedText.Length);

            return translatedText;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "【翻译】翻译失败 | 文本={Text}", text);
            return text;
        }
    }
}
