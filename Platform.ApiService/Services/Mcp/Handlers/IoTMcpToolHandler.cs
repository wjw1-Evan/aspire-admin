...
public class IoTMcpToolHandler : McpToolHandlerBase
{
    ...
    public IoTMcpToolHandler(IIoTService iotService, ILogger<IoTMcpToolHandler> logger)
    {
        ...
        RegisterTool("get_iot_gateways", "获取物联网网关列表。关键词：物联网,网关,网桥,gateway",
            ...
            async (args, uid) => { var keyword = args.GetValueOrDefault("keyword")?.ToString(); var (Current, PageSize) = ParsePaginationArgs(args); var pageParams = new Platform.ServiceDefaults.Models.ProTableRequest { Current = Current, PageSize = PageSize, Search = keyword }; var result = await _iotService.GetGatewaysAsync(pageParams); var items = await result.Queryable.ToListAsync(); return new { items = items, rowCount = result.RowCount, currentPage = result.CurrentPage, pageSize = result.PageSize, pageCount = result.PageCount }; });
        ...
        RegisterTool("get_iot_devices", "获取物联网设备列表。关键词：物联网,设备,器械,传感器,探头",
            ...
            async (args, uid) => { var gatewayId = args.GetValueOrDefault("gatewayId")?.ToString(); var keyword = args.GetValueOrDefault("keyword")?.ToString(); var (Current, PageSize) = ParsePaginationArgs(args); var pageParams = new Platform.ServiceDefaults.Models.ProTableRequest { Current = Current, PageSize = PageSize, Search = keyword }; var result = await _iotService.GetDevicesAsync(pageParams, gatewayId); var items = await result.Queryable.ToListAsync(); return new { items = items, rowCount = result.RowCount, currentPage = result.CurrentPage, pageSize = result.PageSize, pageCount = result.PageCount }; });
        ...
        RegisterTool("get_iot_datapoints", "获取指定设备的所有观测指标/数据点。关键词：数据点,观测指标,遥测项",
            ...
            async (args, uid) =>
            {
                ...
                var (Current, PageSize) = ParsePaginationArgs(args, defaultPageSize: 50);
                var pageParams = new Platform.ServiceDefaults.Models.ProTableRequest { Current = Current, PageSize = PageSize };
                return await _iotService.GetDataPointsAsync(pageParams, deviceId);
            });
