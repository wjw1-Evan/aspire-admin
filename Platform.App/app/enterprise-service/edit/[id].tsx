...
export default function EditEnterpriseServiceScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [requestRes, tenantRes] = await Promise.all([
        enterpriseService.getRequestById(id),
        enterpriseService.getTenantList({ page: 1 }),
      ]);
      if (requestRes.success && requestRes.data) {
        const d = requestRes.data;
        setRequest(d);
        setDescription(d.description || '');
        setContactPerson(d.contactPerson || '');
        setContactPhone(d.contactPhone || '');
        setPriority(d.priority || 'Normal');
        setTenantId(d.tenantId || undefined);
        ...
        setError(requestRes.message || t('enterprise_service.load_failed'));
      }
      if (tenantRes.success && tenantRes.data) {
        setTenants(tenantRes.data.queryable || []);
      }
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await enterpriseService.updateRequest(id, {
      ...
      if (res.success) {
        Toast.show({
          type: 'success',
          text1: t('enterprise_service.update_success'),
        });
        router.back();
      } else {
        Toast.show({
