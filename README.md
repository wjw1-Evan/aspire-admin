...
- **后端服务**：多租户数据访问工厂、JWT + 刷新令牌、图形验证码与登录失败保护、菜单级权限、企业协作与组织架构、任务/项目管理、工作流 + 表单 + 公文审批、云存储/版本/分享/配额、文件协同（版本、外链、权限）、统一通知中心、好友与位置社交、MCP 规则引擎、IoT 平台、密码本（AES-256-GCM）、AI 智能回复与 SSE 实时推送、园区管理（资产/招商/租户/企业服务/ AI 报表）、系统维护脚本与监控。
...
- **园区管理**：资产（楼宇/房源/状态）、招商（线索/跟进/项目/漏斗）、租户（企业/合同/续租/预警）、企业服务（需求/工单/满意度）、统计（多维报表 + AI 生成运营报告）。
...
- **功能模块**：
  ...
  - **园区管理**：资产管理（楼宇/房源）、招商管理（线索/项目）、租户管理（合同/租户库）、企业服务、统计报表 + AI 分析。
