...
- **全栈 AI 协同**：内置 AI 助手与 MCP (Model Context Protocol) 规则引擎，支持 SSE 长连接流式输出、消息上下文管理、附件协同与附近的人/社交会话。
...
- **项目与任务**：支持任务依赖链条、执行日志追踪、项目成员权限隔离与全局看板统计。
- **SSE 实时通信**：高性能长连接通道，支持系统通知实时下发、在线状态感知与流式 AI 对话。
- **密码本 (Key Vault)**：用户级 AES-256-GCM 高强度加密存储，支持动态生成、强度审计与分类管理。
- **系统监控**：暴露底层资源占用（CPU/内存/磁盘）API，支持 DataInitializer 反馈初始化状态。

## 🖥 管理后台 (Platform.Admin)

- **底层技术**：React 19, Ant Design 6 (原生组件), @umijs/max, Biome 规范校验。
- **精美仪表盘**：重构的欢迎页面集成统计概览、任务状态、最近活动与系统资源实时监控图表。
- **多语言适配**：内置 18 种语言翻译包，支持主题切换与响应式布局。
- **主要模块**：
  - **园区中心**：资产画像、招商漏斗、租户档案、合同预警、企业服务。
  - **协作中心**：云存储库（版本/分享）、工作流看板、公文处理中心、项目任务树。
  - **开发者中心**：IoT 网关/设备监控、规则配置中心、MCP 自动化工作流、系统日志审计。

## 📱 移动应用 (Platform.App)

- **核心场景**：实时移动办公审批、AI 聊天辅助、同事圈/附近的人（LBS）、附件在线预览与统一通知触达。
- **技术特色**：Expo Router 文件式路由、主题化 UI 系统、离线网络异常处理。
...
3. **配置 OpenAI** (可选)
   - 在 `Platform.AppHost` 设置相应的 Endpoint 与 Key 以启用 AI 对话功能。
...
- `Platform.ApiService/Controllers/`: 涵盖 Auth, Park, IoT, Workflow, CloudStorage, Chat 等 40+ 控制器。
- `Platform.Admin/src/pages/`: 包含各模块的所有管理页面、设计器与工作流交互页面。
- `docs/`: 存放详细的开发规范、API 说明、权限策略与安全审计报告。
