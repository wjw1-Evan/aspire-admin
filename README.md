...
本项目深度集成了 **MCP 协议**，使 AI 助手能够安全地访问企业内部的核心业务逻辑。目前有 **30 个 MCP 工具处理器**，覆盖系统所有核心业务模块。
...
- **云盘与配额管理**
   - **云盘管理 (CloudStorage)**：文件列表、文件夹创建、文件操作（重命名/移动/复制/删除）、搜索、最近文件、回收站
   - **存储配额 (StorageQuota)**：我的配额、用户配额、配额列表、使用统计、企业统计、排行、警告、空间检查

- **AI 智能助手**
   - **AI 聊天 (ChatAi)**：会话列表、创建会话、会话详情、消息列表、发送消息、标记已读、删除消息、智能回复、匹配推荐
   - **小科配置 (XiaokeConfig)**：配置列表/详情/创建/更新/删除/设置默认 + 聊天历史列表/详情/删除
