...
### 10.10 工具调用的用户体验设计（新增）

#### 工具响应的用户感知

MCP 工具的执行结果直接影响用户对 AI 能力的第一印象：

| 场景 | 好的用户体验 | 差的用户体验 |
|------|------------|------------|
| 工具执行成功 | 返回清晰的结构化数据 + 下一步建议 | 返回原始 JSON，让用户自己理解 |
| 工具执行失败 | "操作失败：任务不存在" + 建议输入有效 ID | `{success: false}` 没有说明原因 |
| 参数缺失 | "请提供任务名称或 ID" + 引导用户补充 | 什么都不做，没有任何反馈 |
| 权限不足 | "您没有操作 XX 的权限" + 建议联系管理员 | 返回 403 错误码 |
| 操作耗时 | 先返回"正在处理..."的提示 | 用户以为系统没有响应 |

#### 确认流程设计

对于有副作用的操作（删除、修改、批量操作），工具应返回确认提示：
...
{
    // 检测是否为危险操作
    if (IsDeleteOperation(parameters))
    {
        return new
        {
            requiresConfirmation = true,
            message = $"确定要删除「{entityName}」吗？此操作不可撤销",
            entityId = entityId
        };
    }
...
- 返回给用户的信息**不要包含技术细节**（堆栈、异常类型），而是**用户能理解的描述**
- 长列表结果应提供摘要（共 23 条记录，显示前 5 条）
- 复杂操作的结果应包含关键字段的说明

#### 关键用户体验检查清单

开发每个 MCP Handler 时，问自己：

- [ ] **用户能看懂返回值吗？** 还是需要 AI 额外解释一次？
- [ ] **操作失败时，** 用户知道哪里错了、怎么修正吗？
- [ ] **参数不全时，** 能引导用户补充而不是沉默失败吗？
- [ ] **危险操作，** 提供了确认流程吗？
- [ ] **返回大量数据时，** 做了摘要优化吗？
