...
- **使用示例**：
  ```csharp
  [Route("api/rules")]
  [RequireMenu("rule:list")]
  public class RuleController : BaseApiController
  {
      [HttpGet]
      [RequireMenu("rule:list")] // 可加在类或方法上
      public async Task<IActionResult> GetList([FromQuery] RuleQueryParams query)
      {
...
- **命名建议**：
  - 一般采用 `模块:资源:操作` 的形式，例如：
    - `rule:list`、`rule:create`、`rule:update`、`rule:delete`
    - `user:list`、`user:invite`、`user:disable`
  - 对于纯页面级菜单，可只使用 `模块:页`：如 `dashboard:analysis`。
- **前后端统一**：
