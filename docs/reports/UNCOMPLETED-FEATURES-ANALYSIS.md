...
### 1. 验证码功能 - 假实现 🔴 严重（已移除前端 Mock 数据）

**最新进展（2025-11-03）**:
- 已删除 `Platform.Admin/mock` 目录，移除所有前端 Mock 登录接口
- 登录页现直接调用后端 `/api/login/captcha` 与 `/api/login/verify-captcha` 接口

**剩余问题**:
- 需完善图形验证码的防刷策略与交互（如滑块、点选）
- 建议集成短信服务或更加安全的验证码触达方式

**注意**: 虽已移除 Mock 数据，验证码生成与发送策略仍需评估其生产安全性。
