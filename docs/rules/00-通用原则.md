...
### 审计与软删除（自动处理，禁止重复实现）
- **软删除**：`DbContext.Remove()` 由 `PlatformDbContext` 自动转换为软删除，业务代码**禁止**手动设置 `IsDeleted`、`DeletedAt`、`DeletedBy`。
- **多租户过滤**：查询时由 `PlatformDbContext` 全局查询过滤器自动过滤 `CompanyId` 和 `IsDeleted=false`，业务代码**禁止**手动添加过滤条件。
- **审计字段**：创建/更新/删除通过 `DbContext.Set<T>()` 操作并调用 `SaveChangesAsync()`，审计字段由 `PlatformDbContext` 自动维护，业务代码**禁止**手动赋值。
...
### 分页查询
- 调用 `Skip((page-1)*pageSize).Take(pageSize)` 执行分页。
...
- [`docs/rules/backend/README.md`](./backend/README.md) - 后端详细规则索引
