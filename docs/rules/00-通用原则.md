...
- **多租户自动过滤**：实现 `IMultiTenant` 的实体由 `PlatformDbContext` 自动附加 `CompanyId` 过滤。
...
### 审计与软删除
- 创建/更新/删除通过 `DbContext.Set<T>()` 操作并调用 `SaveChangesAsync()`。
- 审计字段由 `PlatformDbContext` 在保存时自动维护，业务代码不得手填。
