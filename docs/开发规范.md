...
- 后端：.NET 10 + Aspire，EF Core + MongoDB Provider (基于 IDataFactory)，GridFS，YARP 网关，OpenTelemetry，SSE 实时通信，OpenAI 客户端。
...
- 授权与租户：权限粒度=菜单，使用 `[RequireMenu("module:resource:action")]`；涉及到多租户隔离的业务实体 **必须** 继承 `MultiTenantEntity` 基类；业务上下文仅通过 `ICurrentUserContext` 获取用户/企业/权限；禁止直读 JWT 或直接访问 Mongo 无租户过滤。
- 数据访问：CRUD 必须走 `IDataFactory<T>`；使用 **LINQ 表达式** 构建所有过滤器与更新逻辑；跨租户或查询删除记录必须使用工厂对应方法（如 `FindIncludingDeletedAsync`），**严禁修改 `PlatformDbContext`**，也**严禁**注入任何数据库驱动特定的 Collection/Database 对象或 `DbContext`。所有排序需使用 `Func<IQueryable<T>, IOrderedQueryable<T>>` 或 standard LINQ (OrderBy/ThenBy)。统一使用 `BaseEntity` 提供的 `CreatedBy` 和 `UpdatedBy`（用户ID）进行操作跟踪，不再使用 `CreatedByUsername` 和 `UpdatedByUsername`。
- **性能优化（N+1 防护）**：
