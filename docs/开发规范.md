...
- 后端：.NET 10 + Aspire，EF Core + MongoDB Provider (直接注入 DbContext)，GridFS，YARP 网关，OpenTelemetry，SSE 实时通信，OpenAI 客户端。
...
- 授权与租户：权限粒度=菜单，使用 `[RequireMenu("module:resource:action")]`；涉及到多租户隔离的业务实体 **必须** 继承 `IMultiTenant` 接口；业务上下文通过 `ITenantContext` 获取用户/企业/权限；**禁止直读 JWT** 或直接访问 Mongo 无租户过滤。
- 数据访问：
  - CRUD **必须**通过注入 `DbContext` 使用 `Set<T>()` 操作
  - 使用 **LINQ 表达式** 构建所有过滤器与更新逻辑
  - 跨租户或查询删除记录使用 `IgnoreQueryFilters()`
  - **禁止**注入 `IMongoCollection<T>` 或 `IMongoDatabase` 直接操作 MongoDB
  - 所有排序需使用 `Func<IQueryable<T>, IOrderedQueryable<T>>` 或 standard LINQ (OrderBy/ThenBy)
  - 统一使用 `BaseEntity` 提供的 `CreatedBy` 和 `UpdatedBy`（用户ID）进行操作跟踪，不再使用 `CreatedByUsername` 和 `UpdatedByUsername`
  - `PlatformDbContext` 会自动应用租户过滤器（仅对实现 `IMultiTenant` 的实体生效）和软删除过滤器
- **性能优化（N+1 防护）**：
  - **严禁**在循环内调用单条查询方法
  - **必须**使用批量查询方法
  - DTO 转换应先批量收集 ID，再批量查询，最后内存映射
- **异常处理**：非关键操作异常（如通知发送失败）使用 `LogDebug`；日志应包含上下文信息；禁止 `LogWarning` 泛滥
- **类型命名**：**禁止**使用 `using Xxx = Yyy.Zzz` 别名写法；**必须**直接使用完整类型名（如 `WorkTask`、`Models.TaskStatus`、`AppUser`）
...
- 后端：所有数据操作必须使用 **DbContext** + **LINQ 表达式**，全面实现数据库无关的数据访问层。`PlatformDbContext` 自动应用租户隔离、软删除过滤与审计追踪。控制器返回统一 `ApiResponse<T>`。所有更新操作使用 `Set<T>().Update()` 方法结合 Lambda 表达式。
...
- 新建模块指南：`docs/features/NEW-MODULE-GUIDE.md`
