...
- 数据访问：CRUD 必须走 `IDataFactory<T>`；使用 **LINQ 表达式** 构建所有过滤器与更新逻辑；跨租户或查询删除记录必须使用工厂对应方法（如 `FindIncludingDeletedAsync`），**严禁**注入任何数据库驱动特定的 Collection/Database 对象或 `DbContext`。所有排序需使用 `Func<IQueryable<T>, IOrderedQueryable<T>>` 或 standard LINQ (OrderBy/ThenBy)。统一使用 `BaseEntity` 提供的 `CreatedBy` 和 `UpdatedBy`（用户ID）进行操作跟踪，不再使用 `CreatedByUsername` 和 `UpdatedByUsername`。
