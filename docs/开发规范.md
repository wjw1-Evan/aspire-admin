...
- 授权与租户：权限粒度=菜单，使用 `[RequireMenu("module:resource:action")]`；涉及多企业数据隔离的实体 **必须** 继承 `MultiTenantEntity`；业务上下文仅通过 `ICurrentUserContext` 获取用户/企业/权限；禁止直读 JWT 或直接访问 Mongo 无租户过滤。
