...
- 数据访问：CRUD 必须走 `IDatabaseOperationFactory<T>`；跨租户或查询删除记录必须使用工厂对应方法（如 `FindIncludingDeletedAsync`、`FindWithoutTenantFilterAsync`），**严禁**任何形式的 `IMongoCollection/IMongoDatabase` 直接注入。
...
- 后端：所有数据操作必须使用数据工厂 `IDatabaseOperationFactory<T>`，严禁绕过工厂直接操作数据库；自动应用租户过滤与审计；控制器返回统一 `ApiResponse<T>`。
